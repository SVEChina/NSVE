...
#ifndef IMI_CORE_TYPES_H
#define IMI_CORE_TYPES_H

#include "IMICompileConfig.h"
...
strings
*/
#if defined(_IMI_WCHAR_FILESYSTEM)
typedef wchar_t fschar_t;
#define _IMI_TEXT(X) L##X
#else
typedef char fschar_t;
#define _IMI_TEXT(X) X
#endif
...
#if defined(IMI_WINDOWS_API) && defined(_MSC_VER) && !defined (_WIN32_WCE)
#if defined(WIN64) || defined(_WIN64) // using portable common solution for x64 configuration
#include <crtdbg.h>
#define _IMI_DEBUG_BREAK_IF( _CONDITION_ ) if (_CONDITION_) {_CrtDbgBreak();}
#else
#define _IMI_DEBUG_BREAK_IF( _CONDITION_ ) if (_CONDITION_) {_asm int 3}
#endif
#else
#include "assert.h"
#define _IMI_DEBUG_BREAK_IF( _CONDITION_ ) assert( !(_CONDITION_) );
#endif
#else
#define _IMI_DEBUG_BREAK_IF(_CONDITION_)
#endif

//! Defines a deprecated macro which generates a warning at compile time
/** The usage is simple
For typedef:		typedef _IMI_DEPRECATED_ int test1;
For classes/structs:	class _IMI_DEPRECATED_ test2 { ... };
For methods:		class test3 { _IMI_DEPRECATED_ virtual void foo() {} };
For functions:		template<class T> _IMI_DEPRECATED_ void test4(void) {}
**/

#if defined(IGNORE_DEPRECATED_WARNING)
#define _IMI_DEPRECATED_
#elif _MSC_VER >= 1310 //vs 2003 or higher
#define _IMI_DEPRECATED_ __declspec(deprecated)
#elif (__GNUC__ > 3 || (__GNUC__ == 3 && __GNUC_MINOR__ >= 1)) // all versions above 3.0 should support this feature
#define _IMI_DEPRECATED_  __attribute__ ((deprecated))
#else
#define _IMI_DEPRECATED_
#endif
...
virtual void somefunc() _IMI_OVERRIDE_;
*/
#if (((__GNUC__ > 4) || ((__GNUC__ == 4) && (__GNUC_MINOR__ >= 7))) && (defined(__GXX_EXPERIMENTAL_CXX0X) || __cplusplus >= 201103L))
#define _IMI_OVERRIDE_ override
#elif (_MSC_VER >= 1600) /* supported since MIMIC 2010 */
#define _IMI_OVERRIDE_ override
#elif (__clang_major__ >= 3)
#define _IMI_OVERRIDE_ override
#else
#define _IMI_OVERRIDE_
#endif

// memory debugging
#if defined(_DEBUG) && defined(IMI_EXPORTS) && defined(_MSC_VER) && \
...
#if defined(IMI_WINDOWS_API) && defined(_MSC_VER) && (_MSC_VER >= 1400)
...
//! creates four CC codes used in IMINGINE for simple ids
/** some compilers can create those by directly writing the
code like 'code', but some generate warnings so we use this macro here */
#define MAKE_IMI_ID(c0, c1, c2, c3) \
        ((imi::u32)(imi::u8)(c0) | ((imi::u32)(imi::u8)(c1) << 8) | \
        ((imi::u32)(imi::u8)(c2) << 16) | ((imi::u32)(imi::u8)(c3) << 24 ))
