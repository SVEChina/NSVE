#ifndef __IMI_HASH_H__
#define __IMI_HASH_H__
...
namespace imi {
    ...
    template <class Type,class Counter = s32> class IMIHash {
    protected:
        struct Data;
    public:
        class Iterator {
        private:
            friend class IMIHash;
        ...
        IMIHash &operator=(const IMIHash &h) {
            if(this == &h)
                return *this;
        ...
        sv_inline Type &operator[](s32 id) {
            assert((u32)id < (u32)capacity && data[id].usage && "IMIHash::operator[](): bad id");
            return data[id].data;
        }
        ...
        sv_inline Type &get(s32 id) {
            assert((u32)id < (u32)capacity && data[id].usage && "IMIHash::get(): bad id");
            return data[id].data;
        }
        ...
        s32 append(s32 id,const Type &t) {
            assert(id >= 0 && "IMIHash::append(): bad id");
            if(id_length > 0) {
                for(s32 i = 0; i < id_length; i++) {
                    if(id_data[i] == id) {
        ...
        s32 remove(s32 id) {
            assert((u32)id < (u32)capacity && data[id].usage && "IMIHash::remove(): bad id");
            data[id].usage = 0;
    ...
    /*
     */
    template <class Type,s32 Capacity = 128,class Counter = s32> class IMIHashStack : public IMIHash<Type,Counter> {
    public:
        IMIHashStack() {
            IMIHash<Type,Counter>::capacity = Capacity;
            IMIHash<Type,Counter>::data = stack_data;
            IMIHash<Type,Counter>::id_capacity = Capacity;
            IMIHash<Type,Counter>::id_data = id_stack_data;
            IMIHash<Type,Counter>::clear();
        }
        ~IMIHashStack() { }
        
    private:
        typename IMIHash<Type,Counter>::Data stack_data[Capacity];
