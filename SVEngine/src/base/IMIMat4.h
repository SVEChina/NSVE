#ifndef __IMI_MAT4_H__
#define __IMI_MAT4_H__
...
namespace imi {
    ...
    //************** FMat4 ************************
    IMI_ALIGNED16(struct) FMat4 {
        sv_inline FMat4() {
        }
        
        FMat4(const FMat4 &m);
        
        sv_inline FMat4(cptr8 _value) {
            IMIStringArray t_str_array;
            t_str_array.setData(_value,',');
            for(s32 i=0;i<16;i++) {
                mat[i] = atof(t_str_array[0]);
            }
        }
        ...
        explicit FMat4(const IMIQuat &q);
        explicit FMat4(const f32 *m);
        FMat4(const FMat3 &m,const FVec3 &v);
        FMat4(const IMIQuat &q,const FVec3 &v);
        ...
        void set(const IMIQuat &q);
        void set(const f32 *m);
        void set(const FMat3 &m,const FVec3 &v);
        void set(const IMIQuat &q,const FVec3 &v);
    ...
    //
    void decomposeTransform(const FMat4 &m,FVec4 &position,IMIQuat &rot);
    FMat4 &composeTransform(FMat4 &ret,const FVec4 &position,const IMIQuat &rot);
    void decomposeTransform(const FMat4 &m,FVec3 &position,IMIQuat &rot,FVec3 &scale);
    FMat4 &composeTransform(FMat4 &ret,const FVec3 &position,const IMIQuat &rot,const FVec3 &scale);
