#ifndef __IMI_VEC2_H__
#define __IMI_VEC2_H__

#include "IMIVecbase.h"
...
namespace imi {
    ...
    *
    ...
    IMI_ALIGNED4(struct) HVec2 {
        ...
        sv_inline HVec2(IMIHalf x,IMIHalf y) : x(x), y(y) {
        }
            
        explicit sv_inline HVec2(IMIHalf v) : x(v), y(v) {
        }
        ...
        sv_inline operator IMIHalf*() { return &x; }
        
        sv_inline operator const IMIHalf*() const { return &x; }
        
        sv_inline operator void*() { return &x; }
        
        sv_inline operator const void*() const { return &x; }

        sv_inline IMIHalf &operator[](s32 i) {
            assert((u32)i < 2 && "HVec2::operator[](): bad index");
            return (&x)[i];
        }
            
        sv_inline IMIHalf operator[](s32 i) const {
            assert((u32)i < 2 && "HVec2::operator[](): bad index");
            return (&x)[i];
        }
