...
#ifndef SV_OBJECT_POOL_H
#define SV_OBJECT_POOL_H
...
#define OBJPOOL_DEFAULT_SIZE 100
    ...
    template <class ShareT,class T> class SVObjectPool : SVGBaseEx {
    public:
        SVObjectPool(SVInstPtr _app)
        :SVGBaseEx(_app) {
            m_size = 0;
            m_use = 0;
            m_step = 100;
        ...
        SVObjectPool(SVInstPtr _app,s32 _size,s32 _step)
        :SVGBaseEx(_app) {
            m_size = 0;
            m_use = 0;
            m_step = _step;
    ...
    //DECLARE_SHAREPTR(SVObjectPool);
    
}//!namespace sv


#endif //SV_OBJECT_POOL_H
