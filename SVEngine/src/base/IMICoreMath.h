...
#ifndef IMI_CORE_MATH_H
#define IMI_CORE_MATH_H
...
#if defined(_IMI_SOLARIS_PLATFORM_) || defined(__BORLANDC__) || defined (__BCPLUSPLUS__) || defined (_WIN32_WCE)
...
#undef PI64
    ...
    */
    ...
    // avoid ambiguity when swapping objects of an IMINGINE type that has not
    // it's own swap overload. Otherwise we get conflicts with some compilers
    // in combination with stl.
    template<class T1, class T2>
    inline void swapNative(T1 &a, T2 &b) {
        T1 c(a);
        a = b;
        b = c;
    }
    ...
    union FloatIntUnion32 {
        FloatIntUnion32(f32 f1 = 0.0f) : f(f1) {}
