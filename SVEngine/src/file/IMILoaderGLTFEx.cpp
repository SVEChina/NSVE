...
#include "../core/IMIAnimateMorph.h"

#include "../rendercore/IMIRMeshRes.h"
...
IMILoaderGLTFEx::IMILoaderGLTFEx(IMIInstPtr _app)
:IMIGBaseEx(_app) {
}

IMILoaderGLTFEx::~IMILoaderGLTFEx() {
}

IMISkinNodePtr IMILoaderGLTFEx::loadFromFile(IMIInstPtr _app, cptr8 _filename) {
    tinygltf::TinyGLTF tiny;
    tinygltf::Model t_model;
    std::string t_err,t_warn;
    IMIString tt = _app->m_file_sys->getFileFullName(_filename);
    std::string t_filename = tt.c_str();
    bool t_ret = tiny.LoadASCIIFromFile(&t_model, &t_err, &t_warn, t_filename);
    if(t_ret) {
        //用各种模型接入
        IMIString t_path = _app->m_file_sys->getPath(tt.c_str());
...
//
IMISkinNodePtr IMILoaderGLTFEx::building(IMIInstPtr _app,
                              tinygltf::Model* _model,
                              cptr8 _path) {
    IMISkinNodePtr _nodePtr = MakeSharedPtr<IMISkinNode>(_app);
    //构建skin
    std::vector<IMISkeletonPtr> t_skeArray;
    for(s32 i=0;i<_model->skins.size();i++) {
        IMISkeletonPtr t_ske = _genSkin(_app,_model,i,_path);
        t_skeArray.push_back(t_ske);
    }
    
    //构建动画
     for(s32 i=0;i<_model->animations.size();i++) {
         IMIAnimateSkinPtr t_ani = _genAnimate(_app,_model,i,_path);
         t_ani->setSkes(t_skeArray);
         _nodePtr->addAni(t_ani);
     }
    
    //构建mtl
    std::vector<IMIMtlCorePtr> _mtlPool;
    std::vector<IMISurfacePtr> _surfacePool;
    ...
    //构建MESH
    std::vector<IMIModelPtr> t_modelsPtr{};
    ...
    for(s32 i=0;i<_model->meshes.size();i++) {
        ...
        for (s32 j = 0; j < meshNum; j++) {
            IMIMesh3dPtr meshPtr = t_modelsPtr[i]->getMesh(j);
...
s32 IMILoaderGLTFEx::getSkinIndexForMeshId(IMIInstPtr _app, tinygltf::Model* _model, s32 _meshId) {
...
void IMILoaderGLTFEx::genNode(IMIInstPtr _app,
                             tinygltf::Model* _model,
                             s32 _nodeid,
                             cptr8 _path) {
    ...
    if(t_node->skin>=0) {
        //有骨骼动画
        int i=0;
//        IMINodePtr t_svNode = _buildSkinNode(t_node);
//        if(t_svNode) {
...
//        }
    ...
    if(t_node->mesh>=0) {
        int i=0;
        //无骨骼动画 纯mesh
//                IMINodePtr t_svNode = _build3DNode(t_node);
//                if(t_svNode) {
//                    m_nodeArray.append(t_svNode);
//                }
    }
    if(t_node->camera>=0) {
//                IMINodePtr t_svNode = _buildCameraNode(t_node);
//                if(t_svNode) {
//                    //t_sc->addNode(t_svNode);
//                    m_nodeArray.append(t_svNode);
//                }
...
IMIModelPtr IMILoaderGLTFEx::_genModel(IMIInstPtr _app,
                                     tinygltf::Model* _model,
                                     s32 _index,
                                     cptr8 _path){
    if(_index<0)
    ...
    IMIString t_modelName = t_meshdata->name.c_str();
    IMIModelMgrPtr t_modelMgr = _app->getModelMgr();
    if (t_modelMgr) {
        IMIModelPtr t_model = t_modelMgr->getModel(t_modelName);
        if (t_model) {
            return t_model;
        }
    }
    IMIModelPtr t_model = MakeSharedPtr<IMIModel>(_app);
    ...
    for(s32 i=0;i<t_meshdata->primitives.size();i++) {
        ...
        IMIMesh3dPtr t_mesh = _genMeshPri(_app,_model,t_pri,_path);
        if(t_mesh) {
...
//
s32 IMILoaderGLTFEx::_getAccLen(tinygltf::Accessor* acc) {
    s32 t_len = 0;
    s32 t_size1 = 0;
...
IMIMesh3dPtr IMILoaderGLTFEx::_genMeshPri(IMIInstPtr _app,
                                        tinygltf::Model* _model,
    ...
    //顶点描述
    IMIVertStreamDspPtr vert_dsp = MakeSharedPtr<IMIVertStreamDsp>(E_BFM_SOA);
    ...
    IMIRenderMeshPtr t_rMesh = MakeSharedPtr<IMIRenderMesh>(_app);
    ...
    if(_prim->indices>=0) {
        ...
        if(t_index_p!=nullptr) {
            ...
            if(_acc_index->componentType == TINYGLTF_COMPONENT_TYPE_BYTE) {
                ...
                t_u_size = sizeof(u16);
                IMIDataSwapPtr t_i_dataswap = MakeSharedPtr<IMIDataSwap>();
    ...
    /*
     构建材质
     */
    //IMIMtlCorePtr t_mtl = _buildMtl(_app,_model,_prim);
    IMIMesh3dPtr t_mesh = MakeSharedPtr<IMIMesh3d>(_app);
    t_mesh->setRenderMesh(t_rMesh);
    return t_mesh;
}

IMISkeletonPtr IMILoaderGLTFEx::_genSkin(IMIInstPtr _app,
                                    tinygltf::Model* _model,
                                    s32 _index,
                                    cptr8 _path){
    if(_index<0)
    ...
    IMISkeletonPtr t_ske = MakeSharedPtr<IMISkeleton>();
    ...
    for(s32 i=0;i<t_skindata->joints.size();i++) {
        ...
        IMIBonePtr t_bone = t_ske->getBoneByNodeID(t_nodeid);//获取对应的骨骼
        if(t_bone) {
            t_bone->m_id = i;
...
IMIBonePtr IMILoaderGLTFEx::_genBone(IMIInstPtr _app,
                                   tinygltf::Model* _model,
    ...
    IMIBonePtr t_bone = MakeSharedPtr<IMIBone>();
...
static sv_inline s32 _getInterpolationMode(IMIString _interpolation){
...
IMIAnimateSkinPtr IMILoaderGLTFEx::_genAnimate(IMIInstPtr _app, tinygltf::Model* _model, s32 _aniIndex, cptr8 _path){
    if(_aniIndex<0) {
        return nullptr;
    }
    tinygltf::Animation* _data = &(_model->animations[_aniIndex]);
    IMIAnimateSkinPtr t_ani = MakeSharedPtr<IMIAnimateSkin>(_app,_data->name.c_str());
    //构建轨道
    for(s32 i=0;i<_data->channels.size();i++) {
        ...
        IMIChannelPtr chn_obj = MakeSharedPtr<IMIChannel>();
        ...
        if( t_chn->target_path == "translation") {
            ...
            for(s32 i=0;i<t_acc_in->count;i++) {
                IMIASKeyPosPtr t_askey = MakeSharedPtr<IMIASKeyPos>();
            ...
            for(s32 i=0;i<t_acc_in->count;i++) {
                IMIASKeyRotPtr t_askey = MakeSharedPtr<IMIASKeyRot>();
                t_askey->m_time = *t_p_tim;
                t_p_tim++;
            ...
            for(s32 i=0;i<t_acc_in->count;i++) {
                IMIASKeyScalePtr t_askey = MakeSharedPtr<IMIASKeyScale>();
                t_askey->m_time = *t_p_tim;
                t_p_tim++;
...
IMIMtlCorePtr IMILoaderGLTFEx::_genMtl(IMIInstPtr _app, tinygltf::Model* _model, s32 _index, cptr8 _path) {
    tinygltf::Material* _mat = &(_model->materials[_index]);
    IMIMtlCorePtr _mtlPtr = _app->getMtlLib()->getMtl("test1");
...
IMISurfacePtr IMILoaderGLTFEx::_genSurface(IMIInstPtr _app, tinygltf::Model* _model, s32 _index, cptr8 _path) {
    ...
    while (it2!=_material->additionalValues.end()) {
        IMIString t_key = it2->first.c_str();
...
//            tinygltf::Parameter* t_param = &(it2->second);
//            if(t_param ) {
//                IMIString t_str = t_param->string_value;
//            }
        }else if(t_key == "name") {
            //材质名称
            tinygltf::Parameter* t_param = &(it2->second);
...
IMITexturePtr IMILoaderGLTFEx::_genTexture(IMIInstPtr _app,
                                         tinygltf::Model* _model,
                                         s32 _index,
                                         cptr8 _path) {
    IMITexturePtr t_tex = nullptr;
    tinygltf::Texture* texture = &(_model->textures[_index]);
    tinygltf::Image* image = &(_model->images[texture->source]);
    if(image->uri.empty()) {
        t_tex = nullptr;
    }else{
        IMIString t_full = _path;
...
s8* IMILoaderGLTFEx::_getAccDataPointer(tinygltf::Model* _model,
...
////                    //
////                    if( t_sampler->minFilter == IMIGLTF_TEXTURE_FILTER_NEAREST) {
////
////                    }else if( t_sampler->minFilter == IMIGLTF_TEXTURE_FILTER_LINEAR) {
////
////                    }else if( t_sampler->minFilter == IMIGLTF_TEXTURE_FILTER_NEAREST_MIPMAP_NEAREST) {
////
////                    }else if( t_sampler->minFilter == IMIGLTF_TEXTURE_FILTER_LINEAR_MIPMAP_NEAREST) {
////
////                    }else if( t_sampler->minFilter == IMIGLTF_TEXTURE_FILTER_NEAREST_MIPMAP_LINEAR) {
////
////                    }else if( t_sampler->minFilter == IMIGLTF_TEXTURE_FILTER_LINEAR_MIPMAP_LINEAR) {
////
////                    }
////                    //
////                    if( t_sampler->magFilter == IMIGLTF_TEXTURE_FILTER_NEAREST) {
////
////                    }else if( t_sampler->magFilter == IMIGLTF_TEXTURE_FILTER_LINEAR) {
////
////                    }else if( t_sampler->magFilter == IMIGLTF_TEXTURE_FILTER_NEAREST_MIPMAP_NEAREST) {
////
////                    }else if( t_sampler->magFilter == IMIGLTF_TEXTURE_FILTER_LINEAR_MIPMAP_NEAREST) {
////
////                    }else if( t_sampler->magFilter == IMIGLTF_TEXTURE_FILTER_NEAREST_MIPMAP_LINEAR) {
////
////                    }else if( t_sampler->magFilter == IMIGLTF_TEXTURE_FILTER_LINEAR_MIPMAP_LINEAR) {
////
////                    }
////                    //
////                    if( t_sampler->wrapS == IMIGLTF_TEXTURE_WRAP_REPEAT ) {
////                        tMtl->setTextureParam(0, E_T_PARAM_WRAP_S, E_T_WRAP_REPEAT);
////                    }else if( t_sampler->wrapS == IMIGLTF_TEXTURE_WRAP_CLAMP_TO_EDGE ) {
////                        tMtl->setTextureParam(0, E_T_PARAM_WRAP_S, E_T_WRAP_CLAMP_TO_EDAGE);
////                    }else if( t_sampler->wrapS == IMIGLTF_TEXTURE_WRAP_MIRRORED_REPEAT ) {
////                        tMtl->setTextureParam(0, E_T_PARAM_WRAP_S, E_T_WRAP_MIRROR);
////                    }
////                    //
////                    if( t_sampler->wrapT == IMIGLTF_TEXTURE_WRAP_REPEAT ) {
////                        tMtl->setTextureParam(0, E_T_PARAM_WRAP_T, E_T_WRAP_REPEAT);
////                    }else if( t_sampler->wrapT == IMIGLTF_TEXTURE_WRAP_CLAMP_TO_EDGE ) {
////                        tMtl->setTextureParam(0, E_T_PARAM_WRAP_T, E_T_WRAP_CLAMP_TO_EDAGE);
////                    }else if( t_sampler->wrapT == IMIGLTF_TEXTURE_WRAP_MIRRORED_REPEAT ) {
////                        tMtl->setTextureParam(0, E_T_PARAM_WRAP_T, E_T_WRAP_MIRROR);
////                    }
////                    //
////                    if( t_sampler->wrapR == IMIGLTF_TEXTURE_WRAP_REPEAT ) {
////                        //tMtl->setTextureParam(0, E_T_PARAM_WRAP_T, E_T_WRAP_REPEAT);
////                    }else if( t_sampler->wrapR == IMIGLTF_TEXTURE_WRAP_CLAMP_TO_EDGE ) {
////                        //tMtl->setTextureParam(0, E_T_PARAM_WRAP_T, E_T_WRAP_CLAMP_TO_EDAGE);
////                    }else if( t_sampler->wrapR == IMIGLTF_TEXTURE_WRAP_MIRRORED_REPEAT ) {
////                        //tMtl->setTextureParam(0, E_T_PARAM_WRAP_T, E_T_WRAP_MIRROR);
////                    }
//                }
