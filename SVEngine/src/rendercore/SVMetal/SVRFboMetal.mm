...
void SVRFboMetal::create(SVRendererPtr _renderer,SVTargetDsp* _logic_dsp) {
    ...
    if(t_rm && m_logic_dsp) {
        ...
        s32 m_width = m_logic_dsp->m_width;
        s32 m_height = m_logic_dsp->m_height;
        s32 m_target_num = m_logic_dsp->m_target_num>SV_SUPPORT_MAX_TAREGT ? SV_SUPPORT_MAX_TAREGT : m_logic_dsp->m_target_num;
        bool m_use_depth = m_logic_dsp->m_use_depth;
        bool m_use_stencil = m_logic_dsp->m_use_stencil;
        ...
        for(s32 i=0;i<m_target_num;i++) {
            m_color_tex[i]= SV_TEXIN(m_logic_dsp->m_color_texid[i]);
        }
        //
...
void SVRFboMetal::_createCommonBuf(SVRendererMetalPtr _renderer) {
    ...
    t_dsp.width = m_logic_dsp->m_width;
    t_dsp.height = m_logic_dsp->m_height;
    t_dsp.depth  = 1;
    ...
    if(t_rm) {
        ...
        for(s32 i=0;i<SV_SUPPORT_MAX_TAREGT;i++) {
            ...
            if(t_tex && t_tex->getResTex() ) {
                ...
                m_pass.colorAttachments[i].clearColor = MTLClearColorMake(m_logic_dsp->m_color_r,
        ...
        if(m_depth_tex) {
            ...
            m_pass.depthAttachment.clearDepth = m_logic_dsp->m_depth_value;
        }
        //支持模版
        if(m_stencil_tex) {
            ...
            m_pass.stencilAttachment.clearStencil = m_logic_dsp->m_stencil_value;
        }
        m_render_encoder = [t_rm->m_cmdBuffer renderCommandEncoderWithDescriptor:m_pass];
        MTLViewport t_vp;
        t_vp.originX = 0.0;
        t_vp.originY = 0.0;
        t_vp.width = m_logic_dsp->m_width;
        t_vp.height = m_logic_dsp->m_height;
        t_vp.znear = 1.0;
        t_vp.zfar = 10000.0;
