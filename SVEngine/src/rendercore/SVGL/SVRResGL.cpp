...
void SVRResGLTex:: create(render::SVRendererPtr _renderer) {
...
void SVRResGLTex::destroy(render::SVRendererPtr _renderer) {
...
void SVRResGLTexWithTexID:: create(render::SVRendererPtr _renderer) {
    SVRObjBase::create(_renderer);
    m_bLoad = true;
    glBindTexture(GL_TEXTURE_2D, m_id);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
...
void SVRResGLTexWithTexID::destroy(render::SVRendererPtr _renderer) {
    SVRObjBase::destroy(_renderer);
    if(m_id>0){
        m_id = 0;
    }
...
void SVRResGLTexiOS::create(render::SVRendererPtr _renderer){
    if(!_renderer)
        return ;
    SVRObjBase::create(_renderer);
#ifdef SV_IOS
...
void SVRResGLTexiOS::destroy(render::SVRendererPtr _renderer){
#ifdef SV_IOS
    if(m_pPixelBuf){
...
void SVRResGLFBO::create(render::SVRendererPtr _renderer){
    SVRObjBase::create(_renderer);
...
void SVRResGLFBO::destroy(render::SVRendererPtr _renderer){
    if(m_fboID>0) {
...
void SVRResGLOutFBO::create(render::SVRendererPtr _renderer){
    //外部设置的FBO 直接使用外部的就好
}

void SVRResGLOutFBO::destroy(render::SVRendererPtr _renderer){
    m_fboID = 0;
}
...
void SVResGLRenderTarget::create(render::SVRendererPtr _renderer) {
...
void SVResGLRenderTarget::destroy(render::SVRendererPtr _renderer) {
    if( m_fboID >0 ) {
        glBindFramebuffer(GL_FRAMEBUFFER, m_fboID);
...
void SVResGLRenderTargetOut::create(render::SVRendererPtr _renderer) {
}

void SVResGLRenderTargetOut::destroy(render::SVRendererPtr _renderer) {
    m_fboID = 0;
    m_colorID = 0;
}
//
...
void SVResGLRenderTexture::create(render::SVRendererPtr _renderer) {
    SVRResGLFBO::create(_renderer);
    m_dirty = true;
}
...
void SVResGLRenderTexture::destroy(render::SVRendererPtr _renderer) {
    if(m_fboID>0) {
        glBindFramebuffer(GL_FRAMEBUFFER, m_fboID);
