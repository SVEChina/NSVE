...
void SVRFboGL::create(SVRendererPtr _renderer,SVTargetDsp* _logic_dsp) {
    ...
    if(t_rm && m_logic_fbo_dsp) {
        ...
        s32 t_width = m_logic_fbo_dsp->m_width;
        s32 t_height = m_logic_fbo_dsp->m_height;
        s32 t_target_num = m_logic_fbo_dsp->m_target_num>SV_SUPPORT_MAX_TAREGT ? SV_SUPPORT_MAX_TAREGT : m_logic_fbo_dsp->m_target_num;
        ...
        //depth
        if(m_logic_fbo_dsp->m_use_depth && m_logic_fbo_dsp->m_use_stencil) {
        ...
        if(m_logic_fbo_dsp->m_use_depth ) {
            glGenRenderbuffers(1, &m_depth_Tex);
            glBindRenderbuffer(GL_RENDERBUFFER, m_depth_Tex);
        ...
        if(m_logic_fbo_dsp->m_use_stencil ) {
...
void SVRFboGL::resize(s32 _width,s32 _height,SVRendererPtr _renderer) {
    if(m_logic_fbo_dsp->m_width!=_width || m_logic_fbo_dsp->m_height!=_height ) {
        m_logic_fbo_dsp->m_width = _width;
        m_logic_fbo_dsp->m_height = _height;
        //颜色重新
        //删除，重新创建
        if(m_depth_Tex == m_stencil_Tex) {
            if(m_depth_Tex!=0) {
            ...
            glGenRenderbuffers(1, &m_depth_Tex);
            glBindRenderbuffer(GL_RENDERBUFFER, m_depth_Tex);
            glRenderbufferStorage(GL_RENDERBUFFER,GL_DEPTH24_STENCIL8,m_logic_fbo_dsp->m_width,m_logic_fbo_dsp->m_height);
            m_depth_Tex = m_stencil_Tex;
            glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, m_depth_Tex);
            glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, m_stencil_Tex);
        }else{
            if(m_depth_Tex>0) {
                ...
                glGenRenderbuffers(1, &m_depth_Tex);
                glBindRenderbuffer(GL_RENDERBUFFER, m_depth_Tex);
                glRenderbufferStorage(GL_RENDERBUFFER,GL_DEPTH_COMPONENT24,m_logic_fbo_dsp->m_width,m_logic_fbo_dsp->m_height);//GL_DEPTH_COMPONENT16
                glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, m_depth_Tex);
            }
            if(m_stencil_Tex>0) {
                glDeleteBuffers(1, &m_stencil_Tex);
                glGenRenderbuffers(1, &m_stencil_Tex);
                glBindRenderbuffer(GL_RENDERBUFFER, m_stencil_Tex);
                glRenderbufferStorage(GL_RENDERBUFFER,GL_STENCIL_INDEX8,m_logic_fbo_dsp->m_width,m_logic_fbo_dsp->m_height);//GL_STENCIL_INDEX16
                glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, m_stencil_Tex);
            }
        }
    }
...
void SVRFboGL::bind(SVRendererPtr _renderer) {
    if(m_logic_fbo_dsp) {
        glBindFramebuffer(GL_FRAMEBUFFER, m_fbo_id);
        glViewport(0, 0, m_logic_fbo_dsp->m_width, m_logic_fbo_dsp->m_height);
        glClearColor(m_logic_fbo_dsp->m_color_r,
                     m_logic_fbo_dsp->m_color_g,
                     m_logic_fbo_dsp->m_color_b,
                     m_logic_fbo_dsp->m_color_a);
        if(m_logic_fbo_dsp->m_use_depth && m_logic_fbo_dsp->m_use_stencil) {
            glClearDepthf(m_logic_fbo_dsp->m_depth_value);
            glClearStencil(m_logic_fbo_dsp->m_stencil_value);
            glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT|GL_STENCIL_BUFFER_BIT);
        }else if(m_logic_fbo_dsp->m_use_depth) {
            glClearDepthf(m_logic_fbo_dsp->m_depth_value);
            glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
        }else if(m_logic_fbo_dsp->m_use_stencil) {
            glClearStencil(m_logic_fbo_dsp->m_stencil_value);
