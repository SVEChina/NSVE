...
IMIRShaderGL::IMIRShaderGL(IMIInstPtr _app)
:IMIRShader(_app){
    m_block = false;
    m_programm = 0;
    m_vs = 0;
...
void IMIRShaderGL::load(IMIRendererPtr _renderer,ShaderDsp* _shader_dsp) {
    IMIRShader::load(_renderer, _shader_dsp);
    if(!m_shader_dsp) {
        return ;
    }
    if( m_shader_dsp->m_dsp & IMI_E_TECH_VS ) {
         m_vs = _loadShader(_renderer,
                            m_shader_dsp,
                            m_shader_dsp->m_vs_fname.c_str(),
                            IMI_E_TECH_VS);
    }
    if( m_shader_dsp->m_dsp & IMI_E_TECH_FS ) {
        m_fs = _loadShader(_renderer,
                           m_shader_dsp,
                           m_shader_dsp->m_fs_fname.c_str(),
                           IMI_E_TECH_FS);
    }
    if( m_shader_dsp->m_dsp & IMI_E_TECH_GS ) {
       m_gs = _loadShader(_renderer,
                          m_shader_dsp,
                          m_shader_dsp->m_gs_fname.c_str(),
                          IMI_E_TECH_GS);
    }
    if( m_shader_dsp->m_dsp & IMI_E_TECH_TSE ) {
        m_tsc = _loadShader(_renderer,
                            m_shader_dsp,
                            m_shader_dsp->m_tse_fname.c_str(),
                            IMI_E_TECH_TSE);
    }
    if( m_shader_dsp->m_dsp & IMI_E_TECH_TSD ) {
...
u32 IMIRShaderGL::_loadShader(IMIRendererPtr _renderer,
                             ShaderDsp* _shader_dsp,
                             cptr8 _filename,
                             s32 _shaderType){
    IMIRendererGLPtr t_renderGL = std::dynamic_pointer_cast<IMIRendererGL>(_renderer);
    if(!t_renderGL) {
        return 0;
    }
    //自动补充版本号
    IMIString _fname = _filename;
    IMIString _aim_str = "code";
    IMIString _new_str = "code3";
    IMIString version_str = "";
    if( t_renderGL->m_gl_version == 2 ) {
        //version_str = "#version 100 es"
    }else if( t_renderGL->m_gl_version == 3 ) {
        version_str = "#version 300 es \n";
        _fname = IMIString::replace(_fname.c_str(),_aim_str.c_str(),_new_str.c_str());
    }
    ...
    s32 t_shader_type = 0;
    if(_shaderType == IMI_E_TECH_VS) {
        t_shader_type = GL_VERTEX_SHADER;
    }else if(_shaderType == IMI_E_TECH_FS) {
        t_shader_type = GL_FRAGMENT_SHADER;
    }else if(_shaderType == IMI_E_TECH_GS) {
#ifdef IMI_OSX
        t_shader_type = GL_GEOMETRY_SHADER;
#endif
    }else if(_shaderType == IMI_E_TECH_TSD) {
#ifdef IMI_OSX
        t_shader_type = GL_TESS_EVALUATION_SHADER;
#endif
    }else if(_shaderType == IMI_E_TECH_TSE) {
#ifdef IMI_OSX
    ...
    if (GL_FALSE == compileErr) {
        ...
        if (logLen > 0) {
            ...
            IMI_LOG_DEBUG("shader compile error log : \n %s fname:%s \n",
                         log,
                         _fname.c_str());
            free(log);
        }
        ...
        IMI_LOG_DEBUG("load shader file %s type %d sucess\n", _filename, _shaderType);
    }
    return t_shader_id;
}

u32 IMIRShaderGL::_createProgram(){
    if (m_vs == 0 || m_fs == 0) {
        return 0;
    }
    s32 t_error = 0;
    ...
    if (linkstatus == GL_FALSE) {
        ...
        if (logLen > 0) {
            ...
            IMI_LOG_DEBUG("link status:%s, error: %d\n", log, t_error);
            free(log);
        }
    } else {
...
void IMIRShaderGL::_clearShaderRes(){
    if(m_vs != 0) {
        glDeleteShader(m_vs);
        m_vs = 0;
    }
...
void IMIRShaderGL::unload() {
    if(m_programm != 0){
        glDeleteProgram(m_programm);
    }
}

bool IMIRShaderGL::active(IMIRendererPtr _renderer) {
...
void IMIRShaderGL::submitParamTbl() {
    if(!m_shader_dsp){
        return ;
    }
    //OpenGL 应该是不分阶段的传递uniform
    for(s32 i=0;i<m_shader_dsp->m_paramtbl.size();i++) {
        IMIParamTblPtr t_tbl = m_shader_dsp->m_paramtbl[i].m_tbl;
        for(s32 i=0;i<t_tbl->m_param_names.size();i++) {
            IMIString t_name = t_tbl->m_param_names[i].c_str();
            s32 t_locate = glGetUniformLocation(m_programm, t_name.c_str());
            if(t_locate<0) {
                continue;
            }
            if( t_tbl->m_param_dsps[i].m_type == IMI_INT) {
            ...
            }else if( t_tbl->m_param_dsps[i].m_type == IMI_FLOAT) {
            ...
            }else if( t_tbl->m_param_dsps[i].m_type == IMI_FVEC2) {
                f32* t_p = (f32*)(t_tbl->m_param_values->getPointer( t_tbl->m_param_dsps[i].m_off));
                if(t_tbl->m_param_dsps[i].m_cnt == 1){
            ...
            }else if( t_tbl->m_param_dsps[i].m_type == IMI_FVEC3) {
                f32* t_p = (f32*)(t_tbl->m_param_values->getPointer( t_tbl->m_param_dsps[i].m_off));
                if(t_tbl->m_param_dsps[i].m_cnt == 1){
            ...
            }else if( t_tbl->m_param_dsps[i].m_type == IMI_FVEC4) {
                f32* t_p = (f32*)(t_tbl->m_param_values->getPointer( t_tbl->m_param_dsps[i].m_off));
                if(t_tbl->m_param_dsps[i].m_cnt == 1){
            ...
            }else if( t_tbl->m_param_dsps[i].m_type == IMI_FMAT2) {
                f32* t_p = (f32*)(t_tbl->m_param_values->getPointer( t_tbl->m_param_dsps[i].m_off));
                if(t_tbl->m_param_dsps[i].m_cnt == 1){
            ...
            }else if( t_tbl->m_param_dsps[i].m_type == IMI_FMAT3) {
                f32* t_p = (f32*)(t_tbl->m_param_values->getPointer( t_tbl->m_param_dsps[i].m_off));
                if(t_tbl->m_param_dsps[i].m_cnt == 1){
                    glUniformMatrix4fv(t_locate,1,GL_FALSE,t_p);
                }else{
            ...
            }else if( t_tbl->m_param_dsps[i].m_type == IMI_FMAT4) {
                f32* t_p = (f32*)(t_tbl->m_param_values->getPointer( t_tbl->m_param_dsps[i].m_off));
                if(t_tbl->m_param_dsps[i].m_cnt == 1){
                    glUniformMatrix4fv(t_locate,1,GL_FALSE,t_p);
                }else{
            ...
            }else if( t_tbl->m_param_dsps[i].m_type == IMI_BLOCK) {
                //传递一个数据块上去啊
            }
        }
    }
