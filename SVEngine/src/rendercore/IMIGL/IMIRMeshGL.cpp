...
IMIRMeshGL::IMIRMeshGL(IMIInstPtr _app)
:IMIRMeshRes(_app)
,m_indexID(0)
,m_instanceID(0){
...
IMIRMeshGL::~IMIRMeshGL() {
}

void IMIRMeshGL::load(IMIRendererPtr _renderer,
                     IMIIndexStreamDspPtr _indexdsp,
                     IMIVertStreamDspPtr _streamdsp,
                     IMIInstStreamDspPtr _instdsp,
                     IMIRMeshDsp* _IMIRMeshDsp) {
    IMIRMeshRes::load(_renderer, _indexdsp, _streamdsp, _instdsp,_IMIRMeshDsp);
        ...
        if(m_vert_dsp) {
            ...
            if(m_vert_dsp->_bufMode == E_BFM_AOS) {
                ...
                for(s32 i=0;i<t_stream_num;i++) {
                    ...
                    IMIDataSwapPtr t_data = m_vert_dsp->m_streamData[t_vft];
                    if(t_data) {
...
s32 IMIRMeshGL::process(IMIRendererPtr _renderer){
    IMIRRes::process(_renderer);
    IMIRendererGLPtr t_rm = std::dynamic_pointer_cast<IMIRendererGL>(_renderer);
    //数据更新
    m_data_lock->lock();
    //index
    ...
    if( m_vert_dsp ) {
        if( m_vert_dsp->_bufMode == E_BFM_AOS ) {
            ...
            for(s32 i=0;i<m_vert_dsp->m_streamDsp.size();i++) {
                ...
                IMIDataSwapPtr t_data = m_vert_dsp->m_streamData[t_vf_type];
                if(t_data) {
...
void IMIRMeshGL::draw(IMIRendererPtr _renderer) {
    if(!m_rmesh_dsp)
        return ;
    //绘制
