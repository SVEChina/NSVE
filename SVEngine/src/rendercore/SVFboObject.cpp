...
void SVFboObject::create(render::SVRendererPtr _renderer){
...
void SVFboObject::destroy(render::SVRendererPtr _renderer) {
    if (m_objFBOPtr) {
...
void SVOutFboObject::create(render::SVRendererPtr _renderer){
    //外部设置的FBO 直接使用外部的就好
    SVRObjBase::create(_renderer);
    render::SVRendererPtr t_renderBasePtr = mApp->getRenderer();
    SVRendererGLPtr t_renderGLPtr = std::dynamic_pointer_cast<SVRendererGL>(t_renderBasePtr);
...
void SVOutFboObject::destroy(render::SVRendererPtr _renderer){
    if (m_objFBOPtr) {
        m_objFBOPtr->destroy(_renderer);
    }
    SVRObjBase::destroy(_renderer);
