...
IMIParticlesWorld::IMIParticlesWorld(IMIInstPtr _app) :IMIParticlesWorldBase(_app) {
    m_particleNode = nullptr;
}

IMIParticlesWorld::~IMIParticlesWorld() {
    m_particleNode = nullptr;
}

void IMIParticlesWorld::setEmitterEnabled(s32 enable) {
    if(!m_particleNode)
        return ;
//    for(u32 i = 0; i < m_particleNode->getChildNum(); i++) {
//        IMINodePtr t_node = m_particleNode->getChild(i);
//        if(strcmp( t_node->getType(), "IMIParticlesNode") == 0) {
...
s32 IMIParticlesWorld::getFields(IMIArray<WorldField> &fields) {
    fields.clear();
//    IMIArrayStack<Node*> nodes;
//    s32 mask = IMIParticles->getPhysicalMask();
//    // physical wind
//    engine.world->getIntersection(IMIParticles->getWorldIMIBoundBox(),Node::PHYSICAL_WIND,nodes);
//    for(s32 i = 0; i < nodes.size(); i++) {
//        PhysicalWind *wind = static_cast<PhysicalWind*>(nodes[i]);
//        if((wind->getPhysicalMask() & mask) == 0) continue;
//        FMat4 transform = FMat4(wind->getIWorldTransform() * translate(IMIParticles->getWorldOffset()));
//        FMat4 itransform = FMat4(translate(-IMIParticles->getWorldOffset()) * wind->getWorldTransform());
...
//    }
//    // physical water
//    engine.world->getIntersection(IMIParticles->getWorldIMIBoundBox(),Node::PHYSICAL_WATER,nodes);
//    for(s32 i = 0; i < nodes.size(); i++) {
//        PhysicalWater *water = static_cast<PhysicalWater*>(nodes[i]);
//        if((water->getPhysicalMask() & mask) == 0) continue;
//        FMat4 transform = FMat4(water->getIWorldTransform() * translate(IMIParticles->getWorldOffset()));
//        FMat4 itransform = FMat4(translate(IMIParticles->getWorldOffset()) * water->getWorldTransform());
//        Field &f = fields.append();
//        f.transform = transform;
...
s32 IMIParticlesWorld::getForces(IMIArray<WorldForce> &forces) {
    forces.clear();
//    IMIArrayStack<Node*> nodes;
//    s32 mask = IMIParticles->getPhysicalMask();
//    // physical forces
//    engine.world->getIntersection(IMIParticles->getWorldIMIBoundBox(),Node::PHYSICAL_FORCE,nodes);
//    for(s32 i = 0; i < nodes.size(); i++) {
//        PhysicalForce *force = static_cast<PhysicalForce*>(nodes[i]);
//        if((force->getPhysicalMask() & mask) == 0) continue;
//        FMat4 itransform = FMat4(translate(-IMIParticles->getWorldOffset()) * force->getWorldTransform());
...
s32 IMIParticlesWorld::getNoises(IMIArray<WorldNoise> &noises) {
    noises.clear();
//    IMIArrayStack<Node*> nodes;
//    s32 mask = IMIParticles->getPhysicalMask();
//    // physical noises
//    engine.world->getIntersection(IMIParticles->getWorldIMIBoundBox(),Node::PHYSICAL_NOISE,nodes);
//    for(s32 i = 0; i < nodes.size(); i++) {
//        PhysicalNoise *noise = static_cast<PhysicalNoise*>(nodes[i]);
//        if((noise->getPhysicalMask() & mask) == 0) continue;
//        FMat4 itransform = FMat4(translate(-IMIParticles->getWorldOffset()) * noise->getWorldTransform());
...
s32 IMIParticlesWorld::getIntersection(const FVec3 &p0,const FVec3 &p1,WorldContact &contact) {
//    s32 mask = IMIParticles->getIntersectionMask(0);
//    // world space points
//    const FVec3 &world_offset = IMIParticles->getWorldOffset();
...
s32 IMIParticlesWorld::getCollision(const FVec3 &point,
                                   const FVec3 &velocity,
                                   f32 radius,
                                   IMIArray<WorldContact> &contacts,
                                   f32 ifps) {
    contacts.clear();
...
//    // shape parameters
//    IMIArrayStack<Shape::Contact> shape_contacts;
//    if(m_particleNode->getParticlesType() == IMIParticles::TYPE_LENGTH) {
...
s32 IMIParticlesWorld::getRandomPoint(FVec3 &ret_point,FVec3 &ret_normal,FVec3 &ret_velocity) {
//    // parent node
//    Node *parent = IMIParticles->getParent();
//    while(parent != NULL && parent->getType() == Node::NODE_DUMMY) {
//        parent = parent->getParent();
//    }
//    // parent node is not an object
//    if(parent == NULL || parent->isObject() == 0) {
//        return IMIParticles->getRandomPoint(ret_point,ret_normal,ret_velocity,0);
//    }
//    // parent node is object
//    Object *object = static_cast<Object*>(parent);
//    // parent node is object IMIParticles
//    if(object->getType() == Node::OBJECT_PARTICLES) {
//        if(object->isEnabled(0) == 0)
//            return 0;
//        if(object->getMaterial(0) == 0)
//            return 0;
//        IMIParticles *p = static_cast<IMIParticlesNode*>(object)->getParticles();
//        if(p->getNumParticles() == 0)
//            return 0;
//        s32 num = engine.game->getRandomInt(0,p->getNumParticles());
//        ret_point = p->getParticlePosition(num) + FVec3(static_cast<IMIParticlesNode*>(object)->getWorldOffset() - IMIParticles->getWorldOffset());
//        ret_normal = vec3_one;
...
//    FMat4 itransform = FMat4(translate(-IMIParticles->getWorldOffset()) * object->getWorldTransform());
