...
#define IMI_BITFONT_MAX_NUM  10
//
IMIBitFontNode::IMIBitFontNode(IMIInstPtr _app)
:IMINode(_app) {
    ntype = "IMIBitFontNode";
    m_texture = nullptr;
    m_fontDirty = false;
    ...
    m_alpha = 1;
    m_atchType = ATCH_MC;
    m_pRenderVertex = MakeSharedPtr<IMIDataSwap>();
    //m_pMesh = MakeSharedPtr<IMIRenderMesh>(mApp);
...
IMIBitFontNode::~IMIBitFontNode() {
    m_pRenderVertex = nullptr;
    m_pMesh = nullptr;
    m_texture = nullptr;
}

void IMIBitFontNode::update(f32 dt) {
    IMINode::update(dt);
//    if(m_fontDirty){
//        m_fontDirty = false;
//        _refresh();
...
//        //材质独立性
//        IMIMtlCorePtr t_mtl = MakeSharedPtr<IMIMtlCore>(mApp, "normal2d_c");
...
void IMIBitFontNode::setTexture(IMITexturePtr _tex){
    if (_tex) {
        m_texture = _tex;
    }
}

void IMIBitFontNode::setTexture(cptr8 _path){
//    if (m_texture) {
...
cptr8 IMIBitFontNode::getTexturePath(){
//    if (m_texture) {
...
void IMIBitFontNode::setAlpha(f32 _alpha){
...
void IMIBitFontNode::render() {
    if ( m_visible ){
//        IMIRenderScenePtr t_rs = mApp->getRenderMgr()->getRenderScene();
//        if (m_pRenderObj && m_pMesh) {
//            m_pRenderObj->pushCmd(t_rs, RST_SOLID_3D, "IMIBitFontNode");
//        }
    }
    IMINode::render();
}

void IMIBitFontNode::setFontSize(f32 _w,f32 _h){
    m_FontWidth = _w;    //物理尺寸宽高
    m_FontHeight = _h;
    m_fontDirty = true;
}

f32 IMIBitFontNode::getFontW(){
    return m_FontWidth;
}

f32 IMIBitFontNode::getFontH(){
    return m_FontHeight;
}

void IMIBitFontNode::setInnerOff(f32 _offXScale,f32 _offYScale){
    m_innerOffXScale = _offXScale;
    m_innerOffYScale = _offYScale;
    m_fontDirty = true;
}

f32 IMIBitFontNode::getInnerOffXScale(){
    return m_innerOffXScale;
}

f32 IMIBitFontNode::getInnerOffYScale(){
    return m_innerOffYScale;
}

void IMIBitFontNode::setText(s32 _number) {
    setText(IMIString::format("%d",_number).c_str());
}

void IMIBitFontNode::setAtcPt(BITFONT_ATCH_PT _type){
    m_atchType = _type;
    m_fontDirty = true;
};

BITFONT_ATCH_PT IMIBitFontNode::getAtcPt(){
    return m_atchType;
}

//重新构建mesh数据
void IMIBitFontNode::setText(cptr8 _text) {
    if(m_Text!=_text){
        m_Text = _text;
        m_fontDirty = true;
    }
...
void IMIBitFontNode::_refresh(){
    if (m_Text.size() > IMI_BITFONT_MAX_NUM) {
        return;
    }
    ...
    //顶点数据
    V2_C_T0 tVerts[IMI_BITFONT_MAX_NUM * 6];
    //更新每个字符的的纹理坐标
    ...
    //
    if (t_Len < IMI_BITFONT_MAX_NUM) {
        for (s32 i = t_Len; i < IMI_BITFONT_MAX_NUM; ++i) {
            tVerts[i * 6 + 0].x = 0;
            tVerts[i * 6 + 0].y = 0;
            tVerts[i * 6 + 1].x = 0;
            tVerts[i * 6 + 1].y = 0;
    ...
    //
    m_pRenderVertex->writeData(&tVerts[0], sizeof(V2_C_T0) * IMI_BITFONT_MAX_NUM * 6);
    m_pMesh->setDrawVertNum(IMI_BITFONT_MAX_NUM * 6);
    m_pMesh->setVertexData(m_pRenderVertex);
}

void IMIBitFontNode::_genMesh(){
    //构建Mehs
    //构建顶点坐标
    //字符大小
    f32 t_total_w = m_FontWidth*IMI_BITFONT_MAX_NUM;
    f32 t_total_h = m_FontHeight;
    f32 t_offx = 0.0f;
    f32 t_offy = 0.0f;
    if(m_atchType == ATCH_LB){
    ...
    //
    V2_C_T0 m_Verts[IMI_BITFONT_MAX_NUM * 6];
    for (u32 i = 0; i < IMI_BITFONT_MAX_NUM; i++) {
...
//    //
//    m_pRenderVertex->writeData(&m_Verts[0], sizeof(V2_C_T0) * IMI_BITFONT_MAX_NUM * 6);
//    m_pMesh->setDrawVertNum(IMI_BITFONT_MAX_NUM * 6);
//    m_pMesh->setVertexData(m_pRenderVertex);
...
void IMIBitFontNode::_genTexcoords(){
    //构建字符表
    m_charTbl.clear();
...
cptr8 IMIBitFontNode::getText(){
    return m_Text.c_str();
}

void IMIBitFontNode::toJSON(RAPIDJSON_NAMESPACE::Document::AllocatorType &_allocator, RAPIDJSON_NAMESPACE::Value &_objValue){
    
}

void IMIBitFontNode::fromJSON(RAPIDJSON_NAMESPACE::Value &_item){
    _fromJsonData(_item);
    if (_item.HasMember("filefont") && _item["filefont"].IsString()) {
        IMIString t_file_name = _item["filefont"].GetString();
        IMIString t_resPath = m_rootPath +  t_file_name;
