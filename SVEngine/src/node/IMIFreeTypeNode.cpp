...
IMIFreeTypeNode::IMIFreeTypeNode(IMIInstPtr _app)
:IMINode(_app) {
    ntype = "IMIFreeTypeNode";
    ...
    m_texture = nullptr;
    m_pMesh = nullptr;
    m_pMtl= MakeSharedPtr<IMIMtlCore>(mApp, "normal2d");
}

IMIFreeTypeNode::~IMIFreeTypeNode() {
    m_pMesh = nullptr;
}

void IMIFreeTypeNode::setText(cptr8 _text) {
...
cptr8 IMIFreeTypeNode::getText() {
    return m_Text.c_str();
}

void IMIFreeTypeNode::update(f32 dt) {
    IMINode::update(dt);
//    if (m_textChange) {
//        _updateTextTexture();
//        m_textChange = false;
...
void IMIFreeTypeNode::render() {
    if ( m_visible ){
//        IMIRenderScenePtr t_rs = mApp->getRenderMgr()->getRenderScene();
//        if (m_renderObject && m_pMesh && m_pMtl) {
//            m_renderObject->pushCmd(t_rs, RST_SOLID_3D, "IMIFreeTypeNode");
//        }
    }
    IMINode::render();
}

void IMIFreeTypeNode::_updateTextTexture() {
    FT_Face t_fface = mApp->getBasicSys()->getFontModule()->getFontFace();
    if(!t_fface)
        return ;
    //trans 转化字符
    IMIArray<s32> TransChar;
    for(s32 i=0;i<m_Text.size();i++){
        u32 t_result_char = 0;
        if ( (m_Text[i] & 0x80) && (u8)m_Text[i] >= 0) {
            //汉字 转换一个utf8 - unicode
            IMIString t_substr = IMIString::substr(m_Text.c_str(),i,3);
            IMIString::utf8ToUnicode(t_substr.c_str(), t_result_char);
            i += 2;
        } else {
    ...
    //
    m_pMesh = IMIGeoGen::genRect(mApp,t_texw,t_texh,m_aabbBox); //矩形
    m_texture = mApp->getTexMgr()->createUnctrlTexture(t_texw, t_texh,GL_RGBA,GL_RGBA);
    IMIDataSwapPtr t_texture_data = MakeSharedPtr<IMIDataSwap>();
