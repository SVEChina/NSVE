...
#define IMI_BMFONT_MAX_NUM  20
#define DEFSPACE 40
//
IMIBMFontNode::IMIBMFontNode(IMIInstPtr _app)
:IMINode(_app) {
    ntype = "IMIBMFontNode";
    m_font = nullptr;
    m_textDirty = true;
...
void IMIBMFontNode::update(f32 dt) {
    IMINode::update(dt);
//    if (!m_font) {
//        return;
//    }
...
void IMIBMFontNode::render() {
    if (m_visible){
//        IMIRenderScenePtr t_rs = mApp->getRenderMgr()->getRenderScene();
//        if (m_pRenderObj && m_pMesh) {
//            m_pRenderObj->pushCmd(t_rs, m_rsType, "IMIBMFontNode");
//        }
    }
    IMINode::render();
}

void IMIBMFontNode::setFont(IMIBMFontPtr _font){
    if (m_font != _font) {
        m_font = _font;
        m_textDirty = true;
    }
...
void IMIBMFontNode::setFont(cptr8 _name){
    IMIFontProcessPtr fontProcess = mApp->getBasicSys()->getFontModule();
    IMIBMFontPtr t_font = fontProcess->getBMFont(_name);
...
void IMIBMFontNode::setText(cptr8 _text){
    m_text = _text;
    m_textDirty = true;
}

cptr8 IMIBMFontNode::getText(){
    return m_text;
}

void IMIBMFontNode::setSpacing(f32 _spacing){
    m_spacing = _spacing;
    m_textDirty = true;
}

void IMIBMFontNode::setFontSize(f32 _size){
    if (_size <= 0.0f) {
        return;
    }
    m_fontSize = _size;
...
f32 IMIBMFontNode::getWidth(){
    if (!m_font) {
        return 0;
    }
    f32 t_scaleX = 1.0f;
    IMINodePtr t_curNode = THIS_TO_SHAREPTR(IMIBMFontNode);
//    while (t_curNode) {
...
void IMIBMFontNode::setAlpha(f32 _alpha){
...
void IMIBMFontNode::_refresh(){
    //顶点数据
    V2_C_T0 tVerts[IMI_BMFONT_MAX_NUM * 6];
    ...
    for( s32 n = 0; n < t_texLen; )
    {
        ...
        IMIBMFont::IMIBMFONTCHARINFO ch = m_font->getChar(charId);
...
void IMIBMFontNode::_genMesh(){
    V2_C_T0 t_Verts[IMI_BMFONT_MAX_NUM * 6];
    for (u32 i = 0; i < IMI_BMFONT_MAX_NUM ; i++) {
    ...
    //
    m_pRenderVertex->writeData(&t_Verts[0], sizeof(V2_C_T0) * IMI_BMFONT_MAX_NUM * 6);
    m_pMesh->setDrawVertNum(IMI_BMFONT_MAX_NUM * 6);
    m_pMesh->setVertexData(m_pRenderVertex);
...
//序列化接口
void IMIBMFontNode::toJSON(RAPIDJSON_NAMESPACE::Document::AllocatorType &_allocator, RAPIDJSON_NAMESPACE::Value &_objValue){
...
void IMIBMFontNode::fromJSON(RAPIDJSON_NAMESPACE::Value &_item){
    _fromJsonData(_item);
    if (_item.HasMember("fntname") && _item["fntname"].IsString()) {
        IMIString t_fntName = _item["fntname"].GetString();
        IMIString t_resPath = m_rootPath +  t_fntName;
        IMIFontProcessPtr fontProcess = mApp->getBasicSys()->getFontModule();
    ...
    if (_item.HasMember("encode") && _item["encode"].IsInt()) {
        if (m_font) {
            m_font->setTextEncoding(IMIFont::IMIFONTTEXTENCODING(_item["encode"].GetInt()));
        }
    }
