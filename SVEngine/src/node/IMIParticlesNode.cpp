#include "IMIParticlesNode.h"
#include "IMIParticlesWorld.h"
...
//********************************* IMIParticlesNode ********************************************
IMIParticlesNode::IMIParticlesNode(IMIInstPtr _app)
:IMINode(_app) {
    ntype = "IMIParticlesNode";
//    m_drawBox = false;
//    m_canSelect = true;
//    m_rsType = RST_SOLID_3D;
//    m_pParticles = MakeSharedPtr<IMIParticles>(mApp);
//    m_pParticlesWorld = MakeSharedPtr<IMIParticlesWorld>(mApp);
...
//    num_triangles = 0;
//    bound_box = m_pParticles->getIMIBoundBox();
//    //
//    m_mtl_particle = MakeSharedPtr<IMIMtlParticleAni>(mApp);
//    m_pMesh = _app->getRenderMgr()->createMeshRObj();
//    m_pMesh->createMesh();
//    m_pMesh->setVertexType(E_VF_PARTICLE);
//    m_pMesh->setDrawMethod(E_DRAW_TRIANGLES);
//    //
//    m_pVertData = MakeSharedPtr<IMIDataSwap>( );
//    m_pIndexData = MakeSharedPtr<IMIDataSwap>();
...
IMIParticlesNode::~IMIParticlesNode() {
...
void IMIParticlesNode::testInit() {
    if( m_pParticles) {
        //
...
void IMIParticlesNode::setDiffuse(IMITexturePtr _texture){
    if (_texture) {
        m_diffuse = _texture;
    }
}

void IMIParticlesNode::setDiffuse(IMIString _texPath){
    IMITexturePtr texture = mApp->getTexMgr()->getTexture(_texPath.c_str(),true);
    if (texture) {
...
IMIParticlesPtr IMIParticlesNode::getParticles() {
    return m_pParticles;
}

void IMIParticlesNode::update(f32 ifps) {
    IMINode::update(ifps);
    //更新矩阵
    update_transform();
    //粒子系统更新
...
//            IMINodePtr t_node = getChild(i);
//            //            if(node->isEnabledSelf() == 0) {
//            //                continue;
//            //            }
//            // generate IMIParticles
//            IMIString type = t_node->getType();
//            if(type == "IMIParticlesNode") {
//                //粒子上挂粒子
//                IMIParticlesNodePtr p = std::static_pointer_cast<IMIParticlesNode>(t_node);
//                if(p->getEmitterType() == IMIParticles::EMITTER_SPARK) {
//                    //火舌
...
//                num_particles++;
//            }
//        } else if(type == "IMIMeshClusterNode") {
//            //            // mesh cluster
//            //            IMIArray<FMat4> transforms;
...
//    //    bound_box = transform * IMIParticles->getIMIBoundBox();
}

void IMIParticlesNode::render() {
    if (!m_visible)
        return;
...
//        m_pRenderObj->pushCmd(t_rs, m_rsType, "IMIParticlesNode");
//    }
//    //渲染虚拟的东西
//    renderVisualizer();
    //
...
//************************************ Transformation **************************************
const FVec3 &IMIParticlesNode::getWorldOffset() const {
    return world_offset;
}

//这个函数是用来更新发射器和粒子的变换矩阵
void IMIParticlesNode::update_transform() {
    // 发射器速度
...
FVec3 IMIParticlesNode::getParticlePosition(s32 num) const {
    return world_offset + FVec3(m_pParticles->getParticlePosition(num));
}

void IMIParticlesNode::getParticleTransforms(IMIArray<FMat4> &transforms) const {
    FMat4 transform = Mat4_identity;
...
void IMIParticlesNode::setPhysicalMask(s32 mask) {
    physical_mask = mask;
}

s32 IMIParticlesNode::getPhysicalMask() const {
    return physical_mask;
}

//************************************** Emitter *****************************
void IMIParticlesNode::setEmitterSync(s32 sync) {
    emitter_sync = sync;
}

s32 IMIParticlesNode::getEmitterSync() const {
    return emitter_sync;
}

void IMIParticlesNode::setEmitterVelocity(const FVec3 &velocity) {
    emitter_velocity = velocity;
}

const FVec3 &IMIParticlesNode::getEmitterVelocity() const {
    return emitter_velocity;
}

void IMIParticlesNode::addEmitterSpark(const FVec3 &point,const FVec3 &normal,const FVec3 &velocity) const {
    return m_pParticles->addEmitterSpark(FVec3(point - world_offset),normal,velocity);
}

//********************************** Forces ************************************
void IMIParticlesNode::setForceTransform(s32 num,const FMat4 &transform) const {
...
FMat4 IMIParticlesNode::getForceTransform(s32 num) const {
    if(m_pParticles->isForceAttached(num)) {
...
//****************************** Noises********************************************
void IMIParticlesNode::setNoiseTransform(s32 num,const FMat4 &transform) const {
...
FMat4 IMIParticlesNode::getNoiseTransform(s32 num) const {
    if(m_pParticles->isNoiseAttached(num)) {
...
//        Image *IMIParticlesNode::getNoiseImage(s32 num) const {
//            return m_pParticles->getNoiseImage(num);
//        }

//********************************* Deflectors *********************************
void IMIParticlesNode::setDeflectorTransform(s32 num,const FMat4 &transform) const {
...
FMat4 IMIParticlesNode::getDeflectorTransform(s32 num) const {
    if(m_pParticles->isDeflectorAttached(num)) {
...
//******************************* Contact points ******************************
s32 IMIParticlesNode::getRandomPoint(FVec3 &ret_point,FVec3 &ret_normal,FVec3 &ret_velocity,s32 surface) {
    s32 num_particles = m_pParticles->getNumParticles();
    if(num_particles) {
...
s32 IMIParticlesNode::getNumTriangles(s32 surface) const {
    return num_triangles;
}

//********************************* Bounds *******************************
const IMIBoundBox &IMIParticlesNode::getIMIBoundBox() const {
    return bound_box;
}

void IMIParticlesNode::renderVisualizer() {
    //    //渲染发射器
...
void IMIParticlesNode::toJSON(RAPIDJSON_NAMESPACE::Document::AllocatorType &_allocator,
    ...
    m_textureName = IMIString::substr(m_texturePath.c_str(), pos+1);
    ...
    _objValue.AddMember("IMIParticlesNode", locationObj, _allocator);
}

void IMIParticlesNode::fromJSON(RAPIDJSON_NAMESPACE::Value &item) {
    ...
    if (item.HasMember("texture") && item["texture"].IsString()) {
        ...
        if (m_textureName.size() > 0) {
            IMIString t_texturePath = m_rootPath + m_textureName;
            IMITexturePtr t_texture = mApp->getTexMgr()->getTexture(t_texturePath.c_str(),true);
...
//********************************* Clone **************************************
//        IMIParticlesNode *IMIParticlesNode::clone() const {
//            return copy(new IMIParticlesNode());
//        }
//
//        IMIParticlesNode *IMIParticlesNode::copy(IMIParticlesNode *node) const {
//            // parameters
...
////********************************** Load/save world ****************************************
//s32 IMIParticlesNode::loadWorld(const Xml *xml) {
//
//    // parameters
...
//}
//
//s32 IMIParticlesNode::saveWorld(Xml *xml) const {
//
//    // parameters
//    if(getParticlesType() != IMIParticles::TYPE_BILLBOARD) xml->addChild("particles_type")->setIntData(getParticlesType());
...
//
//    // emitter
//    if(getEmitterType() != IMIParticles::EMITTER_POINT) xml->addChild("emitter_type")->setIntData(getEmitterType());
...
////********************************** Save/restore state *****************************************
//
//s32 IMIParticlesNode::saveState(const Stream &stream) const {
//
//    // save IMIParticles
//    if(m_pParticles->saveState(stream) == 0) {
//        Log::error("IMIParticlesNode::saveState(): can't save IMIParticles state\n");
//        return 0;
//    }
//
//    // physical mask
...
//}
//
//s32 IMIParticlesNode::restoreState(const Stream &stream) {
//
//    // restore IMIParticles
//    if(m_pParticles->restoreState(stream) == 0) {
//        //Log::error("IMIParticlesNode::restoreState(): can't restore IMIParticles state\n");
//        return 0;
//    }
//
//    // physical mask
