...
IMISpineNode::IMISpineNode(IMIInstPtr _app)
:IMINode(_app) {
    ntype = "IMISpineNode";
//    m_spine = nullptr;
//    m_pRObj = nullptr;
...
void IMISpineNode::setSpine(IMISpinePtr _spine) {
//    if (m_spine == _spine) {
//        return;
//    }
//    if (m_spine) {
...
//    m_spine = _spine;
//    if(!m_pRObj){
//        m_pRObj = MakeSharedPtr<IMIMultMeshMtlRenderObject>();
//    }
//    m_pRObj->clearMesh();
//    //回调函数
...
IMISpinePtr IMISpineNode::getSpine() {
    return m_spine;
}

void IMISpineNode::clearSpine() {
    m_spine = nullptr;
}

void IMISpineNode::setstate(E_ANISTATE _state){
    m_state = _state;
}

E_ANISTATE IMISpineNode::getstate(){
    return m_state;
}

cptr8 IMISpineNode::getCurAniName(){
    return m_cur_aniname.c_str();
}

void IMISpineNode::setCurAniName(cptr8 _name){
    m_cur_aniname = _name;
}

void IMISpineNode::setloop(bool _loop){
    m_loop = _loop;
}

bool IMISpineNode::getloop(){
    return m_loop;
}

bool IMISpineNode::isImmediatelyPlay(){
    return m_immediatelyPlay;
}

void IMISpineNode::setSpineCallback(sv_spine_callback _cb,void* _obj) {
    m_spine_callback = _cb;
    m_p_cb_obj = _obj;
}

//
void IMISpineNode::update(f32 dt) {
//    if (!m_visible)
//        return;
...
//            IMIMtlAni2DPtr t_mtl = MakeSharedPtr<IMIMtlAni2D>(mApp);
...
//            t_mtl->setBlendMode(IMIMtlAni2D::IMI_MTL_BLENDMODE_NORMAL);
//
...
//                    t_mtl->setBlendMode(IMIMtlAni2D::IMI_MTL_BLENDMODE_ADDITIVE);
//                    break;
//                }
//                case SP_BLEND_MODE_MULTIPLY:{
//                    t_mtl->setBlendState(MTL_BLEND_DST_COLOR, MTL_BLEND_ONE_MINUS_SRC_ALPHA);
//                    t_mtl->setBlendMode(IMIMtlAni2D::IMI_MTL_BLENDMODE_MULTIPLY);
//                    break;
//                }
//                case SP_BLEND_MODE_SCREEN:{
//                    t_mtl->setBlendState(MTL_BLEND_ONE, MTL_BLEND_ONE_MINUS_SRC_COLOR);
//                    t_mtl->setBlendMode(IMIMtlAni2D::IMI_MTL_BLENDMODE_SCREEN);
//                    break;
//                }
//                default:{
...
//    if (m_pRObj) {
//        m_pRObj->pushCmd(t_rs, m_rsType, "IMISpineNode");
//    }
    IMINode::render();
}

void IMISpineNode::play(cptr8 _actname) {
...
void IMISpineNode::addAni(cptr8 _actname){
//    if (m_spine) {
...
//开始回调
void IMISpineNode::_spine_start() {
//    _sendAniEvent("ani_play");
//    //回调
//    if( m_spine_callback ){
//        (*m_spine_callback)(THIS_TO_SHAREPTR(IMISpineNode),m_p_cb_obj,1);
//    }
}

//完成回调
void IMISpineNode::_spine_complete() {
//    _sendAniEvent("ani_complete");
//    //回调
//    if( m_spine_callback ){
//        (*m_spine_callback)(THIS_TO_SHAREPTR(IMISpineNode),m_p_cb_obj,2);
//    }
}

//停止回调
void IMISpineNode::_spine_stop() {
//    if (m_state == E_ANISTATE::tANI_STATE_STOP) {
//        return;
//    }
//    m_visible = false;
...
//    //回调
//    if( m_spine_callback ){
//        (*m_spine_callback)(THIS_TO_SHAREPTR(IMISpineNode),m_p_cb_obj,3);
//    }
}

//发送事件
void IMISpineNode::_sendAniEvent(cptr8 _eventName) {
//    IMIString t_eventName = m_name + IMIString("_") + IMIString(_eventName);
//    IMIAnimateEventPtr t_event = MakeSharedPtr<IMIAnimateEvent>();
//    t_event->m_AnimateName = m_spine->getSpineName();
//    t_event->eventName = t_eventName;
//    mApp->getEventMgr()->pushEventToSecondPool(t_event);
}

void IMISpineNode::setAlpha(f32 _alpha){
...
bool IMISpineNode::getBonePosition(f32 &px, f32 &py, cptr8 bonename) {
//    spBone *m_bone = m_spine->findBone(bonename);  //绑定的骨头
//    if (m_bone) {
//        px = m_bone->worldX*m_scale.x;
//        py = m_bone->worldY*m_scale.y;
//        //逐层找父节点，把本地坐标和世界坐标加上
//        IMINodePtr t_curNode = THIS_TO_SHAREPTR(IMISpineNode);
//        while (t_curNode) {
...
bool IMISpineNode::getBoneScale(f32 &sx, f32 &sy, cptr8 bonename){
...
bool IMISpineNode::getBoneRotation(f32 &rotation, cptr8 bonename){
//    spBone *m_bone = m_spine->findBone(bonename);//spSkeleton_findBone(,bonename);           //绑定的骨头
//    if (m_bone) {
...
f32 IMISpineNode::getSlotAlpha(cptr8 bonename) {
...
void IMISpineNode::setAABBBoxScale(f32 _scale){
    m_aabbBox_scale = _scale;
}

void IMISpineNode::_computeAABBBox(){
//    if (m_spine) {
//        IMIBoundBox t_boundingBox = m_spine->m_aabbBox;
//        FVec3 t_min = t_boundingBox.getMin();
//        FVec3 t_max = t_boundingBox.getMax();
//        m_aabbBox = IMIBoundBox(FVec3(t_min.x*m_aabbBox_scale, t_min.y*m_aabbBox_scale, 1.0), FVec3(t_max.x*m_aabbBox_scale, t_max.y*m_aabbBox_scale, 1.0));
//    }
}

//序列化
void IMISpineNode::toJSON(RAPIDJSON_NAMESPACE::Document::AllocatorType &_allocator, RAPIDJSON_NAMESPACE::Value &_objValue){
...
//    _objValue.AddMember("IMISpineNode", locationObj, _allocator);
}

void IMISpineNode::fromJSON(RAPIDJSON_NAMESPACE::Value &item){
//    _fromJsonData(item);
...
//        }
//        IMISpinePtr t_spine = IMISpine::createSpine(mApp, m_rootPath + t_json, m_rootPath + t_atlas, 1.0f, m_enableMipMap);
//        if ( t_spine ) {
//            s32 len = t_atlas.size();
//            s32 pos = t_atlas.rfind('.');
//            IMIString t_spineName = IMIString::substr(t_atlas.c_str(), 0, pos);
