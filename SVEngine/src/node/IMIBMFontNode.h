//
// IMIBMFontNode.h
// IMIngine
// Copyright 2017-2020
// yizhou.Fu
//

#ifndef IMI_BMFONTNODE_H
#define IMI_BMFONTNODE_H

#include "IMINode.h"

namespace imi {
    
    
        //
        class IMIBMFontNode : public IMINode {
            //纹理坐标
            struct FontTexcoords{
                f32 lt_x;
                f32 lt_y;
                f32 rt_x;
                f32 rt_y;
                f32 lb_x;
                f32 lb_y;
                f32 rb_x;
                f32 rb_y;
                f32 w;
                f32 h;
                f32 ox;
                f32 oy;
                f32 a;
                s32 charID;
            };
        public:
            IMIBMFontNode(IMIInstPtr _app);
            
            ~IMIBMFontNode();
            
            virtual void update(f32 dt);
            
            virtual void render();
#pragma warning      这个接口将要废弃的
            void setFont(IMIBMFontPtr _font);
            
            void setFont(cptr8 _name);
            
            void setText(cptr8 _text);
            
            cptr8 getText();
            
            void setSpacing(f32 _spacing);
            
            void setFontSize(f32 _size);
            
            f32 getWidth();
            
            f32 getHeight();
            
            virtual void setAlpha(f32 _alpha);
            //序列化接口
            void toJSON(RAPIDJSON_NAMESPACE::Document::AllocatorType &_allocator, RAPIDJSON_NAMESPACE::Value &_objValue);
            
            void fromJSON(RAPIDJSON_NAMESPACE::Value &_item);
        
        private:
            void _genMesh();
            void _refresh();
            
        protected:
            f32 m_fontSize;
            IMIRenderMeshPtr m_pMesh;
            IMIDataSwapPtr m_pRenderVertex;
            IMITexturePtr m_texture;
            IMIBMFontPtr m_font;
            typedef IMIArray<FontTexcoords> TEXCOORDTBL;
            BITFONT_ATCH_PT m_atchType;
            f32 m_spacing;
            f32 m_alpha;
            IMIString m_text;
            bool m_textDirty;
        };
    
    
}//!namespace imi



#endif //IMI_BMFONTNODE_H
