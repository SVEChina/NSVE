...
IMIMtl3D::IMIMtl3D(IMIInstPtr _app, cptr8 _shader)
:IMIMtlCore(_app, _shader) {
...
IMIMtl3D::IMIMtl3D(IMIMtl3D *_mtl)
:IMIMtlCore(_mtl){
...
IMIMtlCorePtr IMIMtl3D::clone() {
    return PointerSharedPtr<IMIMtl3D>(new IMIMtl3D(this));
}
...
void IMIMtl3D::setLightPositon(FVec3& _lightPos){
    m_lightPos = _lightPos;
}

void IMIMtl3D::setLightColor(FVec3& _lightColor){
    m_lightColor = _lightColor;
}

FVec3& IMIMtl3D::getLightColor(){
    return m_lightColor;
}

FVec3& IMIMtl3D::getLightPos(){
    return m_lightPos;
}

void IMIMtl3D::setAmbientLightIntensit(f32 _intersit){
    m_ambientStrength = _intersit;
}

void IMIMtl3D::setAmbientLightColorFactor(FVec3& _ambientColor){
    m_ambient_color = _ambientColor;
}

f32 IMIMtl3D::getAmbientLightStrength(){
    return m_ambientStrength;
}

FVec3& IMIMtl3D::getAmbientLightColor(){
    return m_ambient_color;
}

void IMIMtl3D::_submitMtl(IMIRendererPtr _render){
...
//    IMICameraNodePtr t_cameraNode = mApp->getCameraMgr()->getMainCamera();
...
void IMIMtl3D::toJSON(RAPIDJSON_NAMESPACE::Document::AllocatorType &_allocator,
...
void IMIMtl3D::fromJSON(RAPIDJSON_NAMESPACE::Value &item){
    //lighting
