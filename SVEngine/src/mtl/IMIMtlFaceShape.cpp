...
IMIMtlFaceShape::IMIMtlFaceShape(IMIInstPtr _app)
:IMIMtlCore(_app,"faceShape"){
...
IMIMtlFaceShape::IMIMtlFaceShape(IMIMtlFaceShape *_mtl)
:IMIMtlCore(_mtl){
...
IMIMtlFaceShape::~IMIMtlFaceShape(){
}

IMIMtlCorePtr IMIMtlFaceShape::clone() {
    return PointerSharedPtr<IMIMtlFaceShape>(new IMIMtlFaceShape(this));
}

void IMIMtlFaceShape::update(f32 dt){
    ...
    if(t_person){
        s32 t_ptNum = 0;
        ST_PT *faceData = (ST_PT*)t_person->getFaceData(t_ptNum, IMI_E_FACEDATA_SIMPLITY);
...
void IMIMtlFaceShape::_submitMtl(IMIRendererPtr _render){
...
void IMIMtlFaceShape::setEyeLarge(f32 fLeftLarge , f32 fRightLarge){
    m_eyeLarge[0] = fLeftLarge;
    m_eyeLarge[1] = fRightLarge;
}

//算法不支持1这个数值，从set上根本上规避
void IMIMtlFaceShape::setFaceSlim(f32 fSlim){
...
void IMIMtlFaceShape::setChinUpSlim(f32 fSlim){
    if (fabs(fSlim - 1.0) < 0.00001){
        fSlim = 1.00001;
    }
    m_faceSlim[2] = fSlim;
...
void IMIMtlFaceShape::setChinMiddleSlim(f32 fSlim){
    if (fabs(fSlim - 1.0) < 0.00001){
        fSlim = 1.00001;
    }
    m_faceSlim[3] = fSlim;
...
void IMIMtlFaceShape::setChinSlim(f32 fSlim){
    if (fabs(fSlim - 1.0) < 0.00001){
        fSlim = 1.00001;
    }
    m_faceSlim[1] = fSlim;
...
FVec2 IMIMtlFaceShape::getEyeLarge(){
    return FVec2(m_eyeLarge[0], m_eyeLarge[1]);
}

f32 IMIMtlFaceShape::getFaceSlim(){
    return m_faceSlim[0];
}

f32 IMIMtlFaceShape::getChinUpSlim(){
    return m_faceSlim[2];
}

f32 IMIMtlFaceShape::getChinMiddleSlim(){
    return m_faceSlim[3];
}

f32 IMIMtlFaceShape::getChinSlim(){
    return m_faceSlim[1];
}
