...
void SVMtlFilterBrilaterl::_submitMtl(render::SVRendererPtr _render) {
    if(m_paramDirty) {
        m_paramDirty = false;
...
void SVMtlFilterBrilaterl2::_submitMtl(render::SVRendererPtr _render) {
    if(m_paramDirty) {
        m_paramDirty = false;
        f32 tt_w = m_texelWidthOffset /mApp->m_pGlobalParam->m_inner_width;
        f32 tt_h = m_texelHeightOffset /mApp->m_pGlobalParam->m_inner_height;
...
void SVMtlGaussian::_submitMtl(render::SVRendererPtr _render) {
    if(m_paramDirty) {
        m_paramDirty = false;
...
void SVMtlBlurFairGus::_submitMtl(render::SVRendererPtr _render){
...
void SVMtlBlur::_submitMtl(render::SVRendererPtr _render) {
    if(m_paramDirty) {
        m_paramDirty = false;
        f32 t_w = mApp->m_pGlobalParam->m_inner_width;
        f32 t_h = mApp->m_pGlobalParam->m_inner_height;
...
void SVMtlBlurFair::_submitMtl(render::SVRendererPtr _render){
...
void SVMtlFairLtraLow::_submitMtl(render::SVRendererPtr _render){
