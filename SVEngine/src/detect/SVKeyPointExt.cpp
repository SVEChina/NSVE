...
void SVKeyPointExt::faceDataForEyes(f32 *faceLandMark, f32 *eyesPts, s32 &eyesPtsCount){
    s32 t_eyePtIndex = 0;
    //脸部竖直方向
    ...
    SVMathHelper::createCurve(t_lefteye_point, 8, 2,t_lefteye_curvePoint, t_lefteye_curvePointNum, true);
...
//            t_eyePtIndex++;
    ...
    SVMathHelper::createCurve(t_righteye_point, 8, 2,t_righteye_curvePoint, t_righteye_curvePointNum, true);
...
void SVKeyPointExt::faceDataForEyeBrow(f32 *faceLandMark, f32 *eyebrowPts, s32 &eyebrowPtsCount){
    s32 t_eyePtIndex = 0;
    //脸部竖直方向
    FVec2 vertical_dir_up = (FVec2(faceLandMark[2*43], faceLandMark[2*43+1]) - FVec2(faceLandMark[2*45], faceLandMark[2*45+1])).normalize();
    FVec2 vertical_dir_down = -vertical_dir_up;
    ...
    SVMathHelper::createCurve(t_lefteyebrow_point, 9, 1,t_lefteyebrow_curvePoint, t_lefteyebrow_curvePointNum, true);
    ...
    SVMathHelper::createCurve(t_righteyebrow_point, 9, 1,t_righteyebrow_curvePoint, t_righteyebrow_curvePointNum, true);
...
void SVKeyPointExt::faceDataForFaceWithHead(f32 *faceLandMark, f32 *facePts, s32 &facePtsCount){
...
void SVKeyPointExt::faceDataForWhole(f32 *faceLandMark, f32 *facePts, s32 &facePtsCount){
    if (faceLandMark == nullptr || facePts == nullptr) {
        return;
    }
    s32 t_facePtsCount = 106;
    ...
    SVMathHelper::createCurve(t_toptip_point, 12, 2,t_toptip_curvePoint, t_toptip_curvePointNum, false);
    ...
    SVMathHelper::createCurve(t_bottomtip_point, 8, 2,t_bottomtip_curvePoint, t_bottomtip_curvePointNum, false);
...
void SVKeyPointExt::faceDataForTune(f32 *faceLandMarkTune, f32 *faceLandMark, f32 *facePts, s32 &facePtsCount){
    ...
    s32 t_lefteye_curvePointNum = 0;
    SVMathHelper::createCurve(t_lefteye_tune_pts, 8, 1,t_lefteye_tune_curvePoint, t_lefteye_curvePointNum, true);
    for (s32 i = 0; i<t_lefteye_curvePointNum; i++) {
    ...
    SVMathHelper::createCurve(t_lefteye_pts, 8, 1,t_lefteye_curvePoint, t_lefteye_curvePointNum, true);
    ...
    s32 t_righteye_curvePointNum = 0   ;
    SVMathHelper::createCurve(t_righteye_tune_pts, 8, 1,t_righteye_tune_curvePoint, t_righteye_curvePointNum, true);
    for (s32 i = 0; i<t_righteye_curvePointNum; i++) {
    ...
    SVMathHelper::createCurve(t_righteye_pts, 8, 1,t_righteye_curvePoint, t_righteye_curvePointNum, true);
...
void SVKeyPointExt::_generateDirPt(V2 &_ptO, V2 &_ptT, V2 &_tar, f32 _len){
...
void SVKeyPointExt::faceDataForFaceTune(SV_S_FACETUNE_RATIO &_attribute, f32 *faceLandMark, s32 keyPtNum, f32 *faceTunePts, s32 &facePtsCount){
...
void SVKeyPointExt::_generateDirPt(V2 &_pt, FVec2 &_dir, V2 &_tar, f32 _len){
    f32 t_len = _len;
...
void SVKeyPointExt::_expandPts(f32 *_keyPts, s32 _keyPtsCount, f32 *_expandPts, s32 &_expandPtsCount, FVec2 &_vertical_dir_up, FVec2 &_horizontal_dir_left, f32 _offLen){
