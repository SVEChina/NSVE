...
#include "../shapes/IMIPhysicsShape.h"
...
IMIPhysicsBodyRigid::IMIPhysicsBodyRigid(IMIInstPtr _app):IMIPhysicsBody(_app) {
...
IMIPhysicsBodyRigid::IMIPhysicsBodyRigid(IMIInstPtr _app , IMIPhysicsShapePtr _shape):IMIPhysicsBody(_app) {
    m_type = E_PHYSICS_BODY_RIGID;
    m_pShape = _shape;
    m_pBody = nullptr;
    m_pMyMotionState = nullptr;
...
IMIPhysicsBodyRigid::~IMIPhysicsBodyRigid() {
}

void IMIPhysicsBodyRigid::setRestitution(f32 _dis){
     m_pBody->setRestitution(btScalar(_dis));
}

void IMIPhysicsBodyRigid::setFriction(f32 _dis){
    m_pBody->setFriction(_dis);
}

void IMIPhysicsBodyRigid::setBindNode(bool isbind){
    m_isBindNode = isbind;
}

void IMIPhysicsBodyRigid::init(){
...
void IMIPhysicsBodyRigid::destroy(){
    if(m_pShape){
        m_pShape->destroy();
        m_pShape = nullptr;
    }
...
void IMIPhysicsBodyRigid::update(f32 _dt){
    btTransform trans;
...
btRigidBody* IMIPhysicsBodyRigid::getBody(){
    return m_pBody;
}

void IMIPhysicsBodyRigid::setApplyCentralForce(FVec3 _pos){
    if(m_pBody){
        m_pBody->applyCentralForce(btVector3(_pos.x,_pos.y,_pos.z));
    }
}

void IMIPhysicsBodyRigid::setApplyTorque(FVec3 _pos){
    if(m_pBody){
        m_pBody->applyTorque(btVector3(_pos.x,_pos.y,_pos.z));
    }
}

void IMIPhysicsBodyRigid::addConstraint(){
...
void IMIPhysicsBodyRigid::setConstraintXY(f32 _x ,f32 _y ,f32 _z){
    btVector3 newPivotB(_x/250.0,(_y+640)/250.0,_z);
    p2p->setPivotB(newPivotB);
}

void IMIPhysicsBodyRigid::removeConstraint(){
    if(p2p){
