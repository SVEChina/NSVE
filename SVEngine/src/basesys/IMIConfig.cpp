...
IMIConfig::IMIConfig(){
    ...
    cameraHeight = 720;
    cameraAngle = 90;
    cameraFormate = IMI_PF_NULL;
    dataoutFormate = IMI_OUT_STEAM_NULL;
...
s32 IMIConfig::getRealCameraWidth() {
    if (cameraAngle == 0)
        return cameraWidth;
...
s32 IMIConfig::getRealCameraHeight() {
    if (cameraAngle == 0)
        return cameraHeight;
    else if (cameraAngle == 90)
        return cameraWidth;
...
//                m_designAdaptmode = (IMI_DESIGN_ADAPT_MODE)t_value.GetInt();
//            }
...
//    }
//    _adaptScale();
    IMI_LOG_ERROR("IMIConfig::_loadConfig\n");
}

void IMIConfig::_adaptScale() {
