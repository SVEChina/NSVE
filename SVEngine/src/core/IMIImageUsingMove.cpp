//
// IMIImageUsingMove.cpp
// IMIngine
// Copyright 2017-2020
// yizhou.Fu
//

#include "IMIImageUsingMove.h"
#include "../work/IMITdCore.h"

using namespace imi;

IMIImageUsingMove::IMIImageUsingMove(){
...
void  IMIImageUsingMove::clearContrl(){
    m_lock->lock();
...
void IMIImageUsingMove::setControl(FVec2 _v){
    m_lock->lock();
    m_controlArray.append(_v);
    m_lock->unlock();
}

void IMIImageUsingMove::setTargetControl(FVec2 _v){
    m_lock->lock();
    m_targetArray.append(_v);
    m_lock->unlock();
}

FVec2 IMIImageUsingMove::MLS(const FVec2& t){
    m_lock->lock();
    ...
    //
    //IMIArray<FVec2>::Iterator iter= m_controlArray.begin();
    //计算各个控制顶点的权重，也就是计算点t到各个顶点的距离1/sqr(d)
    IMIArray<FVec2>::Iterator  iter_control=m_controlArray.begin();
    ...
    IMIArray<f64>::Iterator iter_w=m_weightArray.begin();
    IMIArray<FVec2>::Iterator iter_target=m_targetArray.begin();
