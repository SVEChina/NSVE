#include "IMIParticles.h"
#include "../base/IMISimdLib.h"
//#include "../base/IMIUtils.h"
#include "../base/IMINoise.h"
...
IMIParticles::IMIParticles(IMIInstPtr _app)
:IMIGBaseEx(_app) {
	m_pWorld = nullptr;
	frame = 0;
	type = TYPE_BILLBOARD;
...
s32 IMIParticles::getParticleNum() {
    return m_particles.size();
}

//*********************************** Update ****************************
void IMIParticles::update(f32 ifps) {
	f32 time = ifps;
	// warming particles
	if(frame++ == 0 && warming) {
	...
	contacts.clear();
    //获取世界中与粒子交互的区域和对象
    IMIArray<WorldField> world_fields;   //获取世界中的防守
    IMIArray<WorldForce> world_forces;   //获取世界中的强制区域
    IMIArray<WorldNoise> world_noises;   //获取世界中的噪声区域
    ...
    if(m_pWorld && collision) {
        IMIArray<WorldContact> world_contacts;
        for(s32 i = 0; i < m_particles.size(); i++) {
            Particle &p = m_particles[i];
...
// render particles
s32 IMIParticles::render(const FMat4 &modelview,const FVec3 &camera) {
    // number of particles
    ...
    IMI_LOG_INFO("particle num %d \n",particles_num_vertex*2);
    return num_particles;
}

//发射粒子
void IMIParticles::spawn_particle(Particle &p,f32 k,f32 ifps) {
	//随机一个初始速度
	FVec3 velocity;
...
//spawn 产卵 大量产生
void IMIParticles::spawn_particles(s32 num_particles,f32 offset,f32 ifps,f32 time) {
	offset /= time;
...
//
void IMIParticles::update_particles(IMIArray<WorldField> &world_fields,
                                   IMIArray<WorldForce> &world_forces,
                                   IMIArray<WorldNoise> &world_noises,
                                   s32 iteration,
                                   f32 ifps){
    ...
    if(noises.size()) {
        for(s32 i = 0; i < noises.size(); i++) {
            ...
            IMIImagePtr image = getNoiseImage(i);
            ...
            for(s32 j = 0; j < m_particles.size(); j++) {
                ...
                IMIImage::Pixel pixel = image->get3D(position.x,position.y,position.z);
...
//更新粒子包围盒
void IMIParticles::update_particles_bounds() {
	if(m_particles.size()) {
...
//更新发射器包围
void IMIParticles::update_bounds() {
	// life time
...
void IMIParticles::prerun_particels(){
    if (prerun_time <= 0) {
        return;
    }
...
void IMIParticles::sort_particles(const FVec3 &camera) {
...
//    imi::quickSort(distances.begin(),distances.end(),compare);
}

//
void IMIParticles::create_particles(V3_PARTICLE *vertex,const FMat4 &modelview,const FVec3 &camera) {
	switch(type) {
        case TYPE_BILLBOARD: {
        ...
        default:{
            assert(0 && "IMIParticles::create_particles(): unknown particles type");
        }
	}
}

//
void IMIParticles::create_billboard_particles(V3_PARTICLE *vertex,const FMat4 &modelview,const FVec3 &camera) {
    assert(type == TYPE_BILLBOARD && "IMIParticles::create_billboard_particles(): bad particles type");
...
//
void IMIParticles::create_oriented_particles(V3_PARTICLE *vertex,const FMat4 &modelview,const FVec3 &camera) {
...
//
void IMIParticles::create_point_particles(V3_PARTICLE *vertex,const FMat4 &modelview,const FVec3 &camera) {
...
//
void IMIParticles::create_length_particles(V3_PARTICLE *vertex,const FMat4 &modelview,const FVec3 &camera) {
...
//
void IMIParticles::create_route_particles(V3_PARTICLE *vertex,const FMat4 &modelview,const FVec3 &camera){
...
//
void IMIParticles::create_chain_particles(V3_PARTICLE *vertex,const FMat4 &modelview,const FVec3 &camera) {
	assert(type == TYPE_CHAIN && "IMIParticles::create_chain_particles(): bad particles type");
	s32 num_particles = m_particles.size();
	if(num_particles < 3) {
        // empty particles
		V3_PARTICLE *v = vertex;
...
//**********************************  Parameters ********************************************
void IMIParticles::setSeed(u32 seed) {
	random.setSeed(seed);
}

u32 IMIParticles::getSeed() const {
	return random.getSeed();
}

void IMIParticles::setWorld(IMIParticlesWorldBasePtr w) {
    m_pWorld = w;
}

IMIParticlesWorldBasePtr IMIParticles::getWorld() {
	return m_pWorld;
}

void IMIParticles::setType(s32 t) {
    if(t>=0 && t<NUM_TYPES) {
        type = t;
        update_bounds();
    }
...
s32 IMIParticles::getType() const {
	return type;
}

void IMIParticles::setWarming(s32 w) {
	warming = w;
}

s32 IMIParticles::getWarming() const {
	return warming;
}

void IMIParticles::setDepthSort(s32 sort) {
	depth_sort = sort;
}
	
s32 IMIParticles::getDepthSort() const {
	return depth_sort;
}

void IMIParticles::setVariationX(s32 v) {
	variation_x = v;
}
	
s32 IMIParticles::getVariationX() const {
	return variation_x;
}

void IMIParticles::setVariationY(s32 v) {
	variation_y = v;
}
	
s32 IMIParticles::getVariationY() const {
	return variation_y;
}

void IMIParticles::setTextureAtlas(s32 atlas) {
	texture_atlas = atlas;
}
	
s32 IMIParticles::getTextureAtlas() const {
	return texture_atlas;
}

void IMIParticles::setIntersection(s32 i) {
	intersection = i;
}

s32 IMIParticles::getIntersection() const {
	return intersection;
}

void IMIParticles::setCollision(s32 c) {
	collision = c;
}

s32 IMIParticles::getCollision() const {
	return collision;
}

void IMIParticles::setCulling(s32 c) {
	culling = c;
}

s32 IMIParticles::getCulling() const {
	return culling;
}

void IMIParticles::setSpawnRate(f32 spawn) {
	spawn_rate = max(spawn,0.0f);
}

f32 IMIParticles::getSpawnRate() const {
	return spawn_rate;
}

void IMIParticles::setSpawnScale(f32 scale) {
	spawn_scale = saturate(scale);
}

f32 IMIParticles::getSpawnScale() const {
	return spawn_scale;
}

void IMIParticles::setSpawnThreshold(f32 threshold) {
	spawn_threshold = max(threshold,0.0f);
}

f32 IMIParticles::getSpawnThreshold() const {
	return spawn_threshold;
}

void IMIParticles::setWorldMass(f32 mass) {
	world_mass = max(mass,0.0f);
	if(world_mass != 0.0f) world_imass = Math::rcp(world_mass);
	else world_imass = 0.0f;
}

f32 IMIParticles::getWorldMass() const {
	return world_mass;
}

void IMIParticles::setLengthStretch(f32 stretch) {
	length_stretch = max(stretch,0.0f);
	update_bounds();
}

f32 IMIParticles::getLengthStretch() const {
	return length_stretch;
}

void IMIParticles::setLinearDamping(f32 damping) {
	linear_damping = max(damping,0.0f);
}

f32 IMIParticles::getLinearDamping() const {
	return linear_damping;
}

void IMIParticles::setAngularDamping(f32 damping) {
	angular_damping = max(damping,0.0f);
}

f32 IMIParticles::getAngularDamping() const {
	return angular_damping;
}

void IMIParticles::setGrowthDamping(f32 damping) {
	growth_damping = max(damping,0.0f);
}

f32 IMIParticles::getGrowthDamping() const {
	return growth_damping;
}

void IMIParticles::setRestitution(f32 r) {
	restitution = saturate(r);
}

f32 IMIParticles::getRestitution() const {
	return restitution;
}

void IMIParticles::setRoughness(f32 r) {
	roughness = saturate(r);
}

f32 IMIParticles::getRoughness() const {
	return roughness;
}

//******************************* Emitter **********************************************
void IMIParticles::updateEmitter() {
	delay_time = delay_mean + random.getFloat(-delay_spread,delay_spread);
	period_time = period_mean + random.getFloat(-period_spread,period_spread);
	duration_time = duration_mean + random.getFloat(-duration_spread,duration_spread);
}

void IMIParticles::setEmitterType(s32 type) {
    if(type>=0 && type<NUM_EMITTERS) {
       emitter_type = type;
    }
}

s32 IMIParticles::getEmitterType() const {
	return emitter_type;
}

void IMIParticles::setEmitterEnabled(s32 enable) {
...
s32 IMIParticles::isEmitterEnabled() const {
	return emitter_enabled;
}

void IMIParticles::setEmitterBased(s32 based) {
	emitter_based = based;
}

s32 IMIParticles::isEmitterBased() const {
	return emitter_based;
}

void IMIParticles::setEmitterShift(s32 shift) {
	emitter_shift = shift;
}

s32 IMIParticles::isEmitterShift() const {
	return emitter_shift;
}

void IMIParticles::setEmitterContinuous(s32 continuous) {
	emitter_continuous = continuous;
}

s32 IMIParticles::isEmitterContinuous() const {
	return emitter_continuous;
}

void IMIParticles::setEmitterSequence(s32 sequence) {
	emitter_sequence = max(sequence,0);
}

s32 IMIParticles::getEmitterSequence() const {
	return emitter_sequence;
}

void IMIParticles::setEmitterLimit(s32 limit) {
	emitter_limit = max(limit,0);
}

s32 IMIParticles::getEmitterLimit() const {
	return emitter_limit;
}

void IMIParticles::setEmitterSize(const FVec3 &size) {
	emitter_size = max(size,FVec3_zero);
	update_bounds();
}

const FVec3 &IMIParticles::getEmitterSize() const {
	return emitter_size;
}

void IMIParticles::setEmitterDirection(const FVec3 &direction) {
	emitter_direction = direction;
	update_bounds();
}

const FVec3 &IMIParticles::getEmitterDirection() const {
	return emitter_direction;
}

void IMIParticles::setEmitterSpread(const FVec3 &spread) {
	emitter_spread = max(spread,FVec3_zero);
	update_bounds();
}

const FVec3 &IMIParticles::getEmitterSpread() const {
	return emitter_spread;
}

void IMIParticles::setEmitterVelocity(const FVec3 &velocity) {
	emitter_velocity = velocity;
}

const FVec3 &IMIParticles::getEmitterVelocity() const {
	return emitter_velocity;
}

void IMIParticles::setEmitterTransform(const FMat4 &transform) {
...
const FMat4 &IMIParticles::getEmitterTransform() const {
	return emitter_transform;
}

void IMIParticles::addEmitterSpark(const FVec3 &point,const FVec3 &normal,const FVec3 &velocity) {
    //fyz
...
//*********************************** Dynamic n***************************
void IMIParticles::setDelay(f32 mean,f32 spread) {
	delay_mean = max(mean,0.0f);
	delay_spread = spread;
    updateEmitter();
}

f32 IMIParticles::getDelayMean() const {
	return delay_mean;
}

f32 IMIParticles::getDelaySpread() const {
	return delay_spread;
}

void IMIParticles::setPrerunTime(f32 time) {
    prerun_time = max(time,0.0f);
}

f32 IMIParticles::getPrerunTime() const {
    return prerun_time;
}

void IMIParticles::setPeriod(f32 mean,f32 spread) {
...
f32 IMIParticles::getPeriodMean() const {
	return period_mean;
}

f32 IMIParticles::getPeriodSpread() const {
	return period_spread;
}

void IMIParticles::setDuration(f32 mean,f32 spread) {
...
f32 IMIParticles::getDurationMean() const {
	return duration_mean;
}

f32 IMIParticles::getDurationSpread() const {
	return duration_spread;
}

void IMIParticles::setLife(f32 mean,f32 spread) {
	life_mean = max(mean,EPSILON);
	life_spread = spread;
	update_bounds();
}

f32 IMIParticles::getLifeMean() const {
	return life_mean;
}

f32 IMIParticles::getLifeSpread() const {
	return life_spread;
}

void IMIParticles::setVelocity(f32 mean,f32 spread) {
	velocity_mean = mean;
	velocity_spread = spread;
	update_bounds();
}

f32 IMIParticles::getVelocityMean() const {
	return velocity_mean;
}

f32 IMIParticles::getVelocitySpread() const {
	return velocity_spread;
}

void IMIParticles::setAngle(f32 mean,f32 spread) {
	angle_mean = mean;
	angle_spread = spread;
}

f32 IMIParticles::getAngleMean() const {
	return angle_mean;
}

f32 IMIParticles::getAngleSpread() const {
	return angle_spread;
}

void IMIParticles::setRotation(f32 mean,f32 spread) {
	rotation_mean = mean;
	rotation_spread = spread;
}

f32 IMIParticles::getRotationMean() const {
	return rotation_mean;
}

f32 IMIParticles::getRotationSpread() const {
	return rotation_spread;
}

void IMIParticles::setRadius(f32 mean,f32 spread) {
	radius_mean = max(mean,EPSILON);
	radius_spread = spread;
	update_bounds();
}

f32 IMIParticles::getRadiusMean() const {
	return radius_mean;
}

f32 IMIParticles::getRadiusSpread() const {
	return radius_spread;
}

void IMIParticles::setFade(f32 _fade){
    fade = _fade;
}
f32 IMIParticles::getFade(){
    return fade;
}

void IMIParticles::setGrowth(f32 mean,f32 spread) {
	growth_mean = mean;
	growth_spread = spread;
	update_bounds();
}

f32 IMIParticles::getGrowthMean() const {
	return growth_mean;
}

f32 IMIParticles::getGrowthSpread() const {
	return growth_spread;
}

//*********************************** Forces *******************************************
void IMIParticles::setGravity(const FVec3 &g) {
	gravity = g;
	update_bounds();
}

const FVec3 &IMIParticles::getGravity() const {
	return gravity;
}

s32 IMIParticles::addForce() {
...
void IMIParticles::removeForce(s32 num) {
    assert(num >= 0 && num < forces.size() && "IMIParticles::removeForce(): bad force number");
    forces.remove(num);
}

s32 IMIParticles::getNumForces() const {
	return forces.size();
}

void IMIParticles::setForceAttached(s32 num,s32 attached) {
	assert(num >= 0 && num < forces.size() && "IMIParticles::setForceAttached(): bad force number");
	forces[num].attached = attached;
}

s32 IMIParticles::isForceAttached(s32 num) const {
	assert(num >= 0 && num < forces.size() && "IMIParticles::isForceAttached(): bad force number");
	return forces[num].attached;
}

void IMIParticles::setForceTransform(s32 num,const FMat4 &transform) {
	assert(num >= 0 && num < forces.size() && "IMIParticles::setForceTransform(): bad force number");
	forces[num].transform = transform;
}

const FMat4 &IMIParticles::getForceTransform(s32 num) const {
	assert(num >= 0 && num < forces.size() && "IMIParticles::getForceTransform(): bad force number");
	return forces[num].transform;
}

void IMIParticles::setForceRadius(s32 num,f32 radius) {
	assert(num >= 0 && num < forces.size() && "IMIParticles::setForceRadius(): bad force number");
	forces[num].radius = max(radius,0.0f);
}

f32 IMIParticles::getForceRadius(s32 num) const {
	assert(num >= 0 && num < forces.size() && "IMIParticles::getForceRadius(): bad force number");
	return forces[num].radius;
}

void IMIParticles::setForceAttenuation(s32 num,f32 attenuation) {
	assert(num >= 0 && num < forces.size() && "IMIParticles::setForceAttenuation(): bad force number");
	forces[num].attenuation = max(attenuation,EPSILON);
}

f32 IMIParticles::getForceAttenuation(s32 num) const {
	assert(num >= 0 && num < forces.size() && "IMIParticles::getForceAttenuation(): bad force number");
	return forces[num].attenuation;
}

void IMIParticles::setForceAttractor(s32 num,f32 attractor) {
	assert(num >= 0 && num < forces.size() && "IMIParticles::setForceAttractor(): bad force number");
	forces[num].attractor = attractor;
}

f32 IMIParticles::getForceAttractor(s32 num) const {
	assert(num >= 0 && num < forces.size() && "IMIParticles::getForceAttractor(): bad force number");
	return forces[num].attractor;
}

void IMIParticles::setForceRotator(s32 num,f32 rotator) {
	assert(num >= 0 && num < forces.size() && "IMIParticles::setForceRotator(): bad force number");
	forces[num].rotator = rotator;
}

f32 IMIParticles::getForceRotator(s32 num) const {
	assert(num >= 0 && num < forces.size() && "IMIParticles::getForceRotator(): bad force number");
	return forces[num].rotator;
}

//*******************************  Noises ***********************************************
s32 IMIParticles::addNoise() {
...
void IMIParticles::removeNoise(s32 num) {
    assert(num >= 0 && num < noises.size() && "IMIParticles::removeNoise(): bad noise number");
    noises.remove(num);
}

s32 IMIParticles::getNumNoises() const {
	return noises.size();
}

void IMIParticles::setNoiseAttached(s32 num,s32 attached) {
	assert(num >= 0 && num < noises.size() && "IMIParticles::setNoiseAttached(): bad noise number");
	noises[num].attached = attached;
}

s32 IMIParticles::isNoiseAttached(s32 num) const {
	assert(num >= 0 && num < noises.size() && "IMIParticles::isNoiseAttached(): bad noise number");
	return noises[num].attached;
}

void IMIParticles::setNoiseTransform(s32 num,const FMat4 &transform) {
	assert(num >= 0 && num < noises.size() && "IMIParticles::setNoiseTransform(): bad noise number");
	noises[num].transform = transform;
}

const FMat4 &IMIParticles::getNoiseTransform(s32 num) const {
	assert(num >= 0 && num < noises.size() && "IMIParticles::getNoiseTransform(): bad noise number");
	return noises[num].transform;
}

void IMIParticles::setNoiseOffset(s32 num,const FVec3 &offset) {
	assert(num >= 0 && num < noises.size() && "IMIParticles::setNoiseOffset(): bad noise number");
	noises[num].offset = offset;
}

const FVec3 &IMIParticles::getNoiseOffset(s32 num) const {
	assert(num >= 0 && num < noises.size() && "IMIParticles::getNoiseOffset(): bad noise number");
	return noises[num].offset;
}

void IMIParticles::setNoiseStep(s32 num,const FVec3 &step) {
	assert(num >= 0 && num < noises.size() && "IMIParticles::setNoiseStep(): bad noise number");
	noises[num].step = step;
}

const FVec3 &IMIParticles::getNoiseStep(s32 num) const {
	assert(num >= 0 && num < noises.size() && "IMIParticles::getNoiseStep(): bad noise number");
	return noises[num].step;
}

void IMIParticles::setNoiseForce(s32 num,f32 force) {
	assert(num >= 0 && num < noises.size() && "IMIParticles::setNoiseForce(): bad noise number");
	noises[num].force = force;
}

f32 IMIParticles::getNoiseForce(s32 num) const {
	assert(num >= 0 && num < noises.size() && "IMIParticles::getNoiseForce(): bad noise number");
	return noises[num].force;
}

void IMIParticles::setNoiseScale(s32 num,f32 scale) {
	assert(num >= 0 && num < noises.size() && "IMIParticles::setNoiseScale(): bad noise number");
	noises[num].scale = saturate(scale);
    noises[num].image = nullptr;
}

f32 IMIParticles::getNoiseScale(s32 num) const {
	assert(num >= 0 && num < noises.size() && "IMIParticles::getNoiseScale(): bad noise number");
	return noises[num].scale;
}

//
void IMIParticles::setNoiseFrequency(s32 num,s32 frequency) {
	assert(num >= 0 && num < noises.size() && "IMIParticles::setNoiseFrequency(): bad noise number");
	noises[num].frequency = clamp(frequency,1,32);
    noises[num].image = nullptr;
}

s32 IMIParticles::getNoiseFrequency(s32 num) const {
	assert(num >= 0 && num < noises.size() && "IMIParticles::getNoiseFrequency(): bad noise number");
	return noises[num].frequency;
}

//
void IMIParticles::setNoiseSize(s32 num,s32 size) {
	assert(num >= 0 && num < noises.size() && "IMIParticles::setNoiseSize(): bad noise number");
	noises[num].size = clamp(size,1,512);
    noises[num].image = nullptr;
}

s32 IMIParticles::getNoiseSize(s32 num) const {
	assert(num >= 0 && num < noises.size() && "IMIParticles::getNoiseSize(): bad noise number");
	return noises[num].size;
}

IMIImagePtr IMIParticles::getNoiseImage(s32 num) {
    assert(num >= 0 && num < noises.size() && "IMIParticles::getNoiseImage(): bad noise number");
    if(noises[num].image == nullptr) {
        Noise &n = noises[num];
        IMINoise x_noise(num);
        IMINoise y_noise(num + 113);
        IMINoise z_noise(num + 49344);
        n.image = MakeSharedPtr<IMIImage>(mApp);
        n.image->create3D(n.size,n.size,n.size,IMI_FORMAT_RGBA8);
...
void IMIParticles::removeDeflector(s32 num) {
    if(num >= 0 && num < deflectors.size()) {
        deflectors.removeForce(num);
    }
}

s32 IMIParticles::getNumDeflectors() const {
	return deflectors.size();
}

void IMIParticles::setDeflectorType(s32 num,s32 type) {
    if(num >= 0 && num < deflectors.size()) {
...
s32 IMIParticles::getDeflectorType(s32 num) const {
	assert(num >= 0 && num < deflectors.size() && "IMIParticles::getDeflectorType(): bad deflector number");
	return deflectors[num].type;
}

void IMIParticles::setDeflectorAttached(s32 num,s32 attached) {
	assert(num >= 0 && num < deflectors.size() && "IMIParticles::setDeflectorAttached(): bad deflector number");
	deflectors[num].attached = attached;
}

s32 IMIParticles::isDeflectorAttached(s32 num) const {
	assert(num >= 0 && num < deflectors.size() && "IMIParticles::isDeflectorAttached(): bad deflector number");
	return deflectors[num].attached;
}

void IMIParticles::setDeflectorTransform(s32 num,const FMat4 &transform) {
	assert(num >= 0 && num < deflectors.size() && "IMIParticles::setDeflectorTransform(): bad deflector number");
	deflectors[num].transform = transform;
}

const FMat4 &IMIParticles::getDeflectorTransform(s32 num) const {
	assert(num >= 0 && num < deflectors.size() && "IMIParticles::getDeflectorTransform(): bad deflector number");
	return deflectors[num].transform;
}

void IMIParticles::setDeflectorSize(s32 num,const FVec3 &size) {
	assert(num >= 0 && num < deflectors.size() && "IMIParticles::setDeflectorSize(): bad deflector number");
	deflectors[num].size = max(size,FVec3_zero);
}

const FVec3 &IMIParticles::getDeflectorSize(s32 num) const {
	assert(num >= 0 && num < deflectors.size() && "IMIParticles::getDeflectorSize(): bad deflector number");
	return deflectors[num].size;
}

void IMIParticles::setDeflectorRestitution(s32 num,f32 restitution) {
	assert(num >= 0 && num < deflectors.size() && "IMIParticles::setDeflectorRestitution(): bad deflector number");
	deflectors[num].restitution = saturate(restitution);
}

f32 IMIParticles::getDeflectorRestitution(s32 num) const {
	assert(num >= 0 && num < deflectors.size() && "IMIParticles::getDeflectorRestitution(): bad deflector number");
	return deflectors[num].restitution;
}

void IMIParticles::setDeflectorRoughness(s32 num,f32 roughness) {
	assert(num >= 0 && num < deflectors.size() && "IMIParticles::setDeflectorRoughness(): bad deflector number");
	deflectors[num].roughness = saturate(roughness);
}

f32 IMIParticles::getDeflectorRoughness(s32 num) const {
	assert(num >= 0 && num < deflectors.size() && "IMIParticles::getDeflectorRoughness(): bad deflector number");
	return deflectors[num].roughness;
}

//序列化接口
void IMIParticles::toJSON(RAPIDJSON_NAMESPACE::Document::AllocatorType &_allocator,
...
void IMIParticles::fromJSON(RAPIDJSON_NAMESPACE::Value &item) {
