...
IMIImage::IMIImage(IMIInstPtr _app)
:IMIGBaseEx(_app){
    m_pData = MakeSharedPtr<IMIDataSwap>();
    clear();
}

IMIImage::IMIImage(IMIInstPtr _app, cptr8 _filename)
:IMIGBaseEx(_app){
    m_pData = MakeSharedPtr<IMIDataSwap>();
    clear();
    load(_filename);
...
IMIImagePtr IMIImage::share() {
    return std::dynamic_pointer_cast<IMIImage>(shareObject()) ;
}

s32 IMIImage::load(cptr8 _filename){
    m_img_name = _filename;
    return IMILoaderImage::load(mApp,share(),_filename);
}
...
void IMIImage::fill(StbRef* _ref) {
    m_width = _ref->width;
    m_height = _ref->height;
    m_depth = 0;//_ref->depth;
    if(_ref->channels == 1) {
        m_type = IMI_IMAGE_2D;
        m_format = IMI_FORMAT_R8;
    }else if(_ref->channels == 2) {
        m_type = IMI_IMAGE_2D;
        m_format = IMI_FORMAT_RG8;
    }else if(_ref->channels == 3) {
        m_type = IMI_IMAGE_2D;
        m_format = IMI_FORMAT_RGB8;
    }else if(_ref->channels == 4) {
        m_type = IMI_IMAGE_2D;
        m_format = IMI_FORMAT_RGBA8;
    }
    //
    m_pData = nullptr;
    m_pData = MakeSharedPtr<IMIDataSwap>();
...
IMITexturePtr IMIImage::toTexture() {
    IMITexturePtr t_tex = MakeSharedPtr<IMITexture>(mApp);
    IMITextureDsp t_dsp;
    t_dsp.m_image_type = IMI_TEXTYPE(m_type);
    ...
    t_dsp.m_srgb = false;
    t_dsp.m_data_formate = IMI_TEXFORMATE(m_format);
...
s32 IMIImage::create2D(s32 _width, s32 _height, s32 _format){
    assert(_width > 0 && _height > 0 && "IMIImage::create2D(): bad image size");
    clear();
    m_type = IMI_IMAGE_2D;
    m_format = _format;
    m_width = _width;
...
s32 IMIImage::create3D(s32 _width, s32 _height, s32 _depth, s32 _format, bool _allocate){
    assert(_width > 0 && _height > 0 && _depth > 0 && "IMIImage::create3D(): bad image size");
    clear();
    m_type = IMI_IMAGE_3D;
    m_format = _format;
    m_width = _width;
...
s32 IMIImage::getPixelSize() const {
    assert(m_format >= IMI_FORMAT_R8 && m_format <= IMI_FORMAT_RGB10A2 && "IMIImage::getPixelSize(): bad image format");
    return pixel_and_block_sizes[(s32)m_format];
}

s32 IMIImage::getWidth() const {
    return m_width;
}

s32 IMIImage::getHeight() const {
    return m_height;
}

s32 IMIImage::getDepth() const {
    return m_depth;
}

s32 IMIImage::isRawFormat() const {
    if(m_format >= IMI_FORMAT_R8 && m_format <= IMI_FORMAT_RGB10A2) {
        return 1;
    }
    return 0;
}

s32 IMIImage::isUCharFormat() const {
    if(m_format >= IMI_FORMAT_R8 && m_format <= IMI_FORMAT_RGBA8) {
        return 1;
    }
    if(m_format >= IMI_FORMAT_DXT1 && m_format <= IMI_FORMAT_PVR4) {
        return 1;
    }
    return 0;
}

s32 IMIImage::isHalfFormat() const {
    if(m_format >= IMI_FORMAT_R16F && m_format <= IMI_FORMAT_RGBA16F) {
        return 1;
    }
    return 0;
}

s32 IMIImage::isFloatFormat() const {
    if(m_format >= IMI_FORMAT_R32F && m_format <= IMI_FORMAT_RGBA32F) {
        return 1;
    }
    return 0;
}

s32 IMIImage::isCombinedFormat() const {
    if(m_format >= IMI_FORMAT_R5G6B5 && m_format <= IMI_FORMAT_RGB10A2) {
        return 1;
    }
    return 0;
}

s32 IMIImage::isCompressedFormat() const {
    if(m_format >= IMI_FORMAT_DXT1 && m_format <= IMI_FORMAT_ZLC2) {
        return 1;
    }
    return 0;
}

s32 IMIImage::isZCompressedFormat() const {
    if(m_format >= IMI_FORMAT_ZLC1 && m_format <= IMI_FORMAT_ZLC2) {
        return 1;
    }
    return 0;
}
...
// loaded flag
s32 IMIImage::isLoaded() const{
    return 1;
}

// image type
s32 IMIImage::getType() const{
    return m_type;
}

cptr8 IMIImage::getTypeName() const{
    return type_names[(s32)m_type];
}

// image format
s32 IMIImage::getFormat() const{
    return m_format;
}

cptr8 IMIImage::getFormatName() const{
    return format_names[(s32)m_format];
}

void IMIImage::set2D(s32 _x, s32 _y, const Pixel &_p){
    assert(m_pData != nullptr && "IMIImage::set2D(): data is NULL");
    assert(isRawFormat() && "IMIImage::set2D(): is not a raw format");
    assert(m_type == IMI_IMAGE_2D && "IMIImage::set2D(): is not a 2D image");
    _set(_x,_y,0,_p);
}

void IMIImage::set2D(s32 _x, s32 _y, s32 _r, s32 _g, s32 _b, s32 _a){
    Pixel p;
    p.i.r = _r;
    p.i.g = _g;
    p.i.b = _b;
...
IMIImage::Pixel IMIImage::get2D(s32 _x, s32 _y) const{
    Pixel p;
    assert(m_pData != NULL && "IMIImage::get2D(): data is NULL");
    assert(isRawFormat() && "IMIImage::get2D(): is not a raw format");
    assert(m_type == IMI_IMAGE_2D && "IMIImage::get2D(): is not a 2D image");
    _get(_x,_y,0,p);
    return p;
}

IMIImage::Pixel IMIImage::get2D(f32 _x, f32 _y) const{
...
s32 IMIImage::get2Di(f32 _x, f32 _y, s32 _index) const{
    s32 p00,p10,p01,p11;
    assert(m_pData != NULL && "Image::get2Di(): data is NULL");
    assert(isRawFormat() && "Image::get2Di(): is not a raw format");
    assert(m_type == IMI_IMAGE_2D && "Image::get2Di(): is not a 2D image");
    f32 x = _x * (m_width - 1);
    f32 y = _y * (m_height - 1);
    s32 x0 = (s32)floorf(x);
    s32 y0 = (s32)floorf(y);
...
void IMIImage::set3D(s32 _x, s32 _y, s32 _z, const Pixel &_p){
    assert(m_pData != nullptr && "IMIImage::set3D(): data is NULL");
    assert(isRawFormat() && "IMIImage::set3D(): is not a raw format");
    assert(m_type == IMI_IMAGE_3D && "IMIImage::set3D(): is not a 3D image");
    _set(_x,_y,_z,0,_p);
}

void IMIImage::set3D(s32 _x, s32 _y, s32 _z, s32 _r, s32 _g, s32 _b, s32 _a){
    Pixel p;
    p.i.r = _r;
    p.i.g = _g;
    p.i.b = _b;
...
IMIImage::Pixel IMIImage::get3D(s32 _x, s32 _y, s32 _z) const{
    Pixel p;
    assert(m_pData != nullptr && "IMIImage::get3D(): data is NULL");
    assert(isRawFormat() && "IMIImage::get3D(): is not a raw format");
    assert(m_type == IMI_IMAGE_3D && "IMIImage::get3D(): is not a 3D image");
    _get(_x,_y,_z,0,p);
    return p;
}

IMIImage::Pixel IMIImage::get3D(f32 _x, f32 _y, f32 _z) const{
...
void IMIImage::setPixels(u8 *pixels, s32 _size){
    if(m_pData) {
        m_pData->writeData(pixels, _size);
    }
}

u8 * IMIImage::getPixels(){
    return (u8 *)m_pData->getData();
}
u8 * IMIImage::getPixels2D(){
    return (u8 *)m_pData->getData();
}
u8 * IMIImage::getPixels3D(){
    return (u8 *)m_pData->getData();
}

cptr8 IMIImage::getPixels() const{
    return (cptr8)m_pData->getData();
}
cptr8 IMIImage::getPixels2D() const{
    return (cptr8)m_pData->getData();
}
cptr8 IMIImage::getPixels3D() const{
    return (cptr8)m_pData->getData();
}

void IMIImage::_set(u32 _x, u32 _y, u32 _offset, const Pixel &_p){
    _x = _x % m_width;
    _y = _y % m_height;
    ...
    switch(m_format) {
        ...
        case IMI_FORMAT_RG16: {
            u16 *d = (u16*)&data[t_id * 4];
            *(d + 0) = (u16)_p.i.r;
            *(d + 1) = (u16)_p.i.g;
            return;
        }
        case IMI_FORMAT_RGB16: {
        ...
        case IMI_FORMAT_R16F: {
            u16 *d = (u16*)&data[t_id * 2];
            *(d + 0) = (u16)_p.i.r;
            return;
        }
        case IMI_FORMAT_RG16F: {
            u16 *d = (u16*)&data[t_id * 4];
            *(d + 0) = (u16)_p.i.r;
            *(d + 1) = (u16)_p.i.g;
            return;
        }
        case IMI_FORMAT_RGB16F: {
            u16 *d = (u16*)&data[t_id * 6];
            *(d + 0) = (u16)_p.i.r;
            *(d + 1) = (u16)_p.i.g;
            *(d + 2) = (u16)_p.i.b;
        ...
        case IMI_FORMAT_R32F: {
            f32 *d = (f32*)&data[t_id * 4];
            *(d + 0) = _p.f.r;
            return;
        }
        case IMI_FORMAT_RG32F: {
            f32 *d = (f32*)&data[t_id * 8];
            *(d + 0) = _p.f.r;
            *(d + 1) = _p.f.g;
            return;
        }
        case IMI_FORMAT_RGB32F: {
...
void IMIImage::_get(u32 _x, u32 _y, u32 _offset, Pixel &_p) const{
    _x = _x % m_width;
    _y = _y % m_height;
    u32 t_id = _offset + m_width * _y + _x;
    u8 *data = (u8 *)m_pData->getData();
...
void IMIImage::_get(u32 _x, u32 _y, u32 _offset, Pixel &_p00, Pixel &_p10, Pixel &_p01, Pixel &_p11) const{
    s32 x0 = _x % m_width;
    s32 y0 = _y % m_height;
    s32 x1 = (_x + 1) % m_width;
...
void IMIImage::_geti(u32 _x, u32 _y, u32 _offset, s32 _index, s32 &_p00, s32 &_p10, s32 &_p01, s32 &_p11) const{
    s32 x0 = _x % m_width;
    s32 y0 = _y % m_height;
    s32 x1 = (_x + 1) % m_width;
    s32 y1 = (_y + 1) % m_height;
...
void IMIImage::_set(u32 _x, u32 _y, u32 _z, u32 _offset, const Pixel &_p){
    _z = _z % m_depth;
    _offset += m_width * m_height * _z;
    _set(_x,_y,_offset,_p);
}

void IMIImage::_get(u32 _x, u32 _y, u32 _z, u32 _offset, Pixel &_p) const{
    _z = _z % m_depth;
    _offset += m_width * m_height * _z;
    _get(_x,_y,_offset,_p);
}

void IMIImage::_get(u32 _x, u32 _y, u32 _z, u32 _offset, Pixel &_p00, Pixel &_p10, Pixel &_p01, Pixel &_p11) const{
    _z = _z % m_depth;
    _offset += m_width * m_height * _z;
    _get(_x,_y,_offset,_p00,_p10,_p01,_p11);
}

void IMIImage::_interpolate(f32 _x, f32 _y, const Pixel &_p00, const Pixel &_p10, const Pixel &_p01, const Pixel &_p11, Pixel &_p) const{
    f32 x1 = 1.0f - _x;
    f32 y1 = 1.0f - _y;
    f32 k00 = x1 * y1;
    f32 k10 = _x * y1;
