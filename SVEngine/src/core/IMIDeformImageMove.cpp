...
IMIParamDeform::IMIParamDeform(){
}

IMIParamDeform::~IMIParamDeform(){
}

IMIDeformImageMove::IMIDeformImageMove(IMIInstPtr _app)
:IMIGBaseEx(_app){
//    m_pMtlBg  = MakeSharedPtr<IMIMtlCore>(mApp,"screen");
//    m_pIUMP    = MakeSharedPtr<IMIImageUsingMove>();
//    m_deformScale = MakeSharedPtr<IMIDeformScale>();
//    m_pPointTex = mApp->getTexMgr()->getTexture("svres/point.png",true);
//    m_param = MakeSharedPtr<IMIParamDeform>();
//    m_param->reset();
...
IMIDeformImageMove::~IMIDeformImageMove(){
    m_passDeform = nullptr;
    m_param=nullptr;
    m_pMeshBg = nullptr;
    m_pMtlBg = nullptr;
...
void IMIDeformImageMove::init(IMITexturePtr _intex,IMITexturePtr _texout){
...
//       
//        _initPoint();
//        t_renderer->createIMITex(E_TEX_FILTER_DEFORM02, m_tt_w, m_tt_h, GL_RGBA);
//        IMITexturePtr t_tex = mApp->getRenderer()->getIMITex(E_TEX_FILTER_DEFORM02);
...
//       
//        //
//        m_passDeform = MakeSharedPtr<IMIPass>();
...
//        
//        IMIMtlCorePtr t_mtl = MakeSharedPtr<IMIMtlCore>(mApp, "normal2dcolor");
...
// 
//        m_passPoint = MakeSharedPtr<IMIPass>();
...
//        
//        t_mtl = MakeSharedPtr<IMIMtlCore>(mApp,"screen");
//        t_mtl->setTexcoordFlip(1.0, -1.0);
//        m_passBack = MakeSharedPtr<IMIPass>();
...
void IMIDeformImageMove::setScaleSmooth(f32 _smooth){
    m_scaleSmooth = _smooth;
    m_deformScale->setSmooth(_smooth);
}


void IMIDeformImageMove::pushScaleCrl(u32 _postion){
    m_scaleCtlArray.append(_postion);
}

void IMIDeformImageMove::clearCrl(){
    m_scaleCtlArray.clear();
}

void IMIDeformImageMove::setPoint(V2 *_data){
    m_dataPoint = _data;
}

void IMIDeformImageMove::updatePointMSL(){
    if(m_dataPoint){
        m_pIUMP->clearContrl();
...
void IMIDeformImageMove::_initPoint(){
//    if(!m_pMeshBg) {
...
//    }
    ...
    //第一次更新索引数据
    IMIDataSwapPtr t_index = MakeSharedPtr<IMIDataSwap>();
...
void IMIDeformImageMove::_refreshScreenRectMesh(V2 *_data,V2 *_targetData){
    if(m_pMeshBg) {
        //渲染数据
...
//        IMIDataSwapPtr t_datav = MakeSharedPtr<IMIDataSwap>();
//        t_datav->writeData(pVer, sizeof(V2_C_T0) * m_wPointCount * m_hPointCont);
//        m_pMeshBg->setDrawVertNum( m_wPointCount * m_hPointCont );
//        m_pMeshBg->setVertexData(t_datav);
    }
}

void IMIDeformImageMove::update(f32 _dt){
    if(!is_swith){
        return;
    }
    ...
    if(is_detect){
        ...
        m_deformScale->clearCtl();
        for(int i=1;i<=4;i++){
            IMIPersonPtr t_person = mApp->getDetectMgr()->getPersonModule()->getPerson(i);
            if( t_person && t_person->getExist()){
                s32 t_ptNum = 0;
                V2 *t_data = (V2*)t_person->getFaceData(t_ptNum, IMI_E_FACEDATA_SIMPLITY);
...
void IMIDeformImageMove::render(){
//    IMIRenderScenePtr t_rs = mApp->getRenderMgr()->getRenderScene();
//    if (is_swith && t_rs && false  == t_rs->isSuspend() ) {
//        if(m_passDeform && m_passDeform->m_pMtl){
//            IMIRCmdPassCollectionPtr t_cmd = MakeSharedPtr<IMIRCmdPassCollection>();
//            t_cmd->mTag = "IMIBackGroundNode";
//            //t_cmd->setFbo(m_fbo);
//            //图片翻转这块需要校正一下 by fyz
//            if(m_pMtlBg){
//                if(m_flip){
...
//            }else{
////                IMITexturePtr t_tex = mApp->getRenderer()->getIMITex(m_passDeform->m_outTexType);
...
s32 IMIDeformImageMove::getWidth(){
    return m_tt_w;
}

s32 IMIDeformImageMove::getHeight(){
    return m_tt_h;
}

void IMIDeformImageMove::setDeformSwitch(bool _swith){
    is_swith=_swith;
}

V2 IMIDeformImageMove::MSL(V2 point){
    if(m_pIUMP) {
...
void IMIDeformImageMove::setTagPoint(u32 _postion,V2 _point){
    m_param->m_pointMap.append(_postion, _point);
}

void IMIDeformImageMove::setTagAreaPoint(u32 _postion,V2 _point){
    m_param->m_areaPoint.append(_postion, _point);
}

void IMIDeformImageMove::pointMove(V2 *t_data){
...
//    m_pIUMP->setTargetControl(FVec2(m_tt_w*0.5f,m_tt_h*0.5f));
    ...
    //迭代偏移值
    IMIMap<u32,V2>::Iterator it = m_param->m_pointMap.begin();
...
void IMIDeformImageMove::_updateControl(V2 *t_data){
    V2 *t_outlinePoints=t_data;
    if(m_smooth<0.01){
        return;
    }
...
void IMIDeformImageMove::_updateMesh(){
...
//serial 序列化接口
void IMIDeformImageMove::toJSON(RAPIDJSON_NAMESPACE::Document::AllocatorType &_aloc,
    ...
    _objValue.AddMember("IMIDeform", t_obj, _aloc);
}

void IMIDeformImageMove::fromJSON(RAPIDJSON_NAMESPACE::Value &item){
    m_param->reset();
...
void IMIDeformImageMove::reset(){
    m_param->reset();
    updatePointMSL();
    updatePointMesh(m_dataPoint);
}

void IMIDeformImageMove::updatePointMesh( V2* _facepoint){
    V2_C_T0 verts[636];
    for(s32 i=0;i<106;i++){
    ...
    IMIDataSwapPtr t_data = MakeSharedPtr<IMIDataSwap>();
    t_data->writeData(&verts[0], sizeof(V2_C_T0) * 636);
//    m_pMeshPoint->setDrawVertNum(636);
//    m_pMeshPoint->setVertexData(t_data);
//    m_pMeshPoint->setVertexType(E_VF_V2_C_T0);
//    m_pMeshPoint->setDrawMethod(E_DRAW_TRIANGLES);
}
