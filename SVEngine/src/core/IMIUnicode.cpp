...
f32 IMIUnicode::decodeUTF8(cptr8 _encodedBuffer, u32 *_outCharLength){
...
f32 IMIUnicode::encodeUTF8(u32 _value, u8 *_outEncodedBuffer, u32 *_outCharLength){
...
f32 IMIUnicode::decodeUTF16(cptr8 _encodedBuffer, u32 *_outCharLength, IMIUNICODEBYTEORDER _byteOrder){
    cptr8 buf = (cptr8)_encodedBuffer;
    int value = 0;
    if( _byteOrder == IMI_LITTLE_ENDIAN )
    {
        value += buf[0];
        value += (s32)(buf[1]) << 8;
    }
...
f32 IMIUnicode::encodeUTF16(u32 _value, u8 *_outEncodedBuffer, u32 *_outCharLength, IMIUNICODEBYTEORDER _byteOrder){
    if( _value < 0x10000 )
    {
        if( _byteOrder == IMI_LITTLE_ENDIAN )
        {
