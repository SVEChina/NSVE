...
//
IMISpinePtr
IMISpine::createSpine(IMIInstPtr _app, cptr8 skefname, cptr8 atlasfname,f32 scale, bool enableMipMap) {
    //
    IMIString t_atlas_fullname = _app->m_file_sys->getFileFullName(atlasfname);
    IMISpineImp* t_imp = new IMISpineImp(_app);
    ...
    json->scale = scale;
    //
    IMIString t_ske_fullname = _app->m_file_sys->getFileFullName(skefname);
    //动画数据
    ...
    IMISpinePtr t_spine = MakeSharedPtr<IMISpine>(_app);
    t_spine->init(skeletonData, _atlas);
    IMIString t_json_file = skefname;
    s32 t_pos = t_json_file.rfind('/');
    s32 t_len = t_json_file.size();
    IMIString t_json_name = IMIString::substr(t_json_file, t_pos+1, t_len - t_pos-1);
    t_spine->m_spine_json = t_json_name;
    IMIString t_atlas_file = atlasfname;
    t_pos = t_atlas_file.rfind('/');
    t_len = t_atlas_file.size();
    IMIString t_atlas_name = IMIString::substr(t_atlas_file, t_pos+1, t_len - t_pos-1);
    t_spine->m_spine_atlas = t_atlas_name;
    return t_spine;
}

IMISpine::IMISpine(IMIInstPtr _app)
:IMIGBaseEx(_app){
...
//初始化
void IMISpine::init(spSkeletonData *skedata, spAtlas *atlas, bool ownsSkeletonData) {
    m_pAtlas = atlas;
...
//        //pMeshData->m_pRenderMesh = MakeSharedPtr<IMIRenderMesh>(mApp);
...
//        pMeshData->m_pRenderIndex  = MakeSharedPtr<IMIDataSwap>();
//        pMeshData->m_pRenderVertex = MakeSharedPtr<IMIDataSwap>();
//        m_spineDataPool.append(pMeshData);
//    }
}

IMITexturePtr IMISpine::_getTexture(spAttachment *pAttach) {
    switch (pAttach->type) {
...
//更新
void IMISpine::update(f32 deltaTime) {
    //更新spine
    ...
    for (s32 i = 0, n = m_pSkeleton->slotsCount; i < n; i++) {
        ...
        IMITexturePtr pTex = _getTexture(t_slot->attachment);
...
void IMISpine::setAnimationStateData(spAnimationStateData *stateData) {
...
//
void IMISpine::getAllAnimationName(IMIArray<IMIString>& _vecName) {
    if(m_pSkeleton && m_pSkeleton->data) {
        for(s32 i=0; i<m_pSkeleton->data->animationsCount; i++){
            IMIString t_name = m_pSkeleton->data->animations[i]->name;
            _vecName.append(t_name);
        }
    }
}

cptr8 IMISpine::getAnimationNameByIndex(s32 _index){
...
spTrackEntry *IMISpine::setAnimationForTrack(s32 trackindex, cptr8 name, bool loop) {
...
spTrackEntry *
IMISpine::addAnimationForTrack(s32 trackindex, cptr8 name, bool loop, s32 delay) {
    spAnimation *animation = spSkeletonData_findAnimation(m_pSkeleton->data, name);
    if (!animation) {
        return nullptr;
    }
...
void IMISpine::clearTracks() {
    spAnimationState_clearTracks(m_pSpineAniState);
}

void IMISpine::clearTrack(s32 trackIndex) {
    spAnimationState_clearTrack(m_pSpineAniState, trackIndex);
}

void IMISpine::setMixFrom(cptr8 fromAnimation, cptr8 toAnimation, f32 duration) {
    spAnimationStateData_setMixByName(m_pSpineAniState->data, fromAnimation, toAnimation, duration);
}

spTrackEntry *IMISpine::getCurrentForTrack(s32 trackIndex) {
    return spAnimationState_getCurrent(m_pSpineAniState, trackIndex);
}

//
IMITexturePtr IMISpine::_getTextureForRegion(spRegionAttachment *_attachment) {
    spAtlasRegion* t_region = ((spAtlasRegion *) _attachment->rendererObject);
    IMISpinePageImp* t_page_imp = (IMISpinePageImp*)(t_region->page->rendererObject);
    if(t_page_imp) {
...
IMITexturePtr IMISpine::_getTextureForMesh(spMeshAttachment *_attachment) {
    spAtlasRegion* t_region = ((spAtlasRegion *) _attachment->rendererObject);
    IMISpinePageImp* t_page_imp = (IMISpinePageImp*)(t_region->page->rendererObject);
    if(t_page_imp) {
        return t_page_imp->m_tex;
...
//render spine
void IMISpine::updateWorldTransform() {
    spSkeleton_updateWorldTransform(m_pSkeleton);
}

void IMISpine::setToSetupPose() {
    spSkeleton_setToSetupPose(m_pSkeleton);
}

void IMISpine::setBonesToSetupPose() {
    spSkeleton_setBonesToSetupPose(m_pSkeleton);
}

void IMISpine::setSlotsToSetupPose() {
    spSkeleton_setSlotsToSetupPose(m_pSkeleton);
}

spBone *IMISpine::findBone(cptr8 _boneName) {
    return spSkeleton_findBone(m_pSkeleton, _boneName);
}

spSlot *IMISpine::findSlot(cptr8 _slotName) {
    return spSkeleton_findSlot(m_pSkeleton, _slotName);
}

bool IMISpine::setSkin(cptr8 _skinName) {
    return (bool) spSkeleton_setSkinByName(m_pSkeleton, _skinName ? _skinName : 0);
}

spAttachment *IMISpine::getAttachment(cptr8 _slotName, cptr8 _attachmentName) {
    return spSkeleton_getAttachmentForSlotName(m_pSkeleton, _slotName, _attachmentName);
}

bool IMISpine::setAttachment(cptr8 _slotName, cptr8 _attachmentName) {
    return (bool) spSkeleton_setAttachment(m_pSkeleton, _slotName, _attachmentName);
}

void IMISpine::setListenerForEntryByStart(spTrackEntry *entry, spStartListener listener) {
//    getListeners(entry)->m_cb_startListener =  listener->copy;
}

void IMISpine::setListenerForEntryByEnd(spTrackEntry *entry, spEndListener listener) {
//    getListeners(entry)->m_cb_endListener = listener.copy();

}

void IMISpine::setListenerForEntryByComp(spTrackEntry *entry, spCompleteListener listener) {
//    getListeners(entry)->m_cb_completeListener = [listener copy];
}

void IMISpine::setListenerForEntryByEvent(spTrackEntry *entry, spEventListener listener) {
//    getListeners(entry)->m_cb_eventListener = [listener copy];
}

void
IMISpine::onAnimationStateEvent(s32 trackIndex, spEventType type, spEvent *event, s32 loopCount) {
    switch (type) {
...
void IMISpine::onTrackEntryEvent(s32 trackIndex, spEventType type, spEvent *event, s32 loopCount) {
...
cptr8 IMISpine::getSpineName() {
    return m_spineName.c_str();
}

void IMISpine::setSpineName(cptr8 name) {
    m_spineName = name;
}

void IMISpine::setAlpha(f32 _alpha){
    m_alpha = _alpha;
}

//序列化接口
void IMISpine::toJSON(RAPIDJSON_NAMESPACE::Document::AllocatorType &_allocator, RAPIDJSON_NAMESPACE::Value &_objValue) {
    
}

void IMISpine::fromJSON(RAPIDJSON_NAMESPACE::Value &item) {
