...
//三角形
IMIRenderMeshPtr IMIGeoGen::genTri(IMIInstPtr _app,
                                     f32 _len,
                                     IMIBoundBox& _aabb) {
...
//方形
IMIRenderMeshPtr IMIGeoGen::genRect(IMIInstPtr _app,f32 _w,f32 _h,IMIBoundBox& _aabb) {
    _aabb.clear();
    //索引
    ...
    //
    IMIRenderMeshPtr t_mesh = MakeSharedPtr<IMIRenderMesh>(_app);
...
IMIRenderMeshPtr IMIGeoGen::genRectARCHO(IMIInstPtr _app,f32 _w,f32 _h,EUIARCHO _archo,IMIBoundBox& _aabb) {
//    s16 t_index_data[] = { 0,1,2,2,1,3 };
//    V3_T0 t_verts[4];
//    //
//    IMIRenderMeshPtr t_mesh = MakeSharedPtr<IMIRenderMesh>(_app);
//    IMIBufferDspPtr t_index_dsp= MakeSharedPtr<IMIBufferDsp>(E_BFM_AOS);
//    IMIRenderMesh::buildIMIBufferDsp(E_BFT_STATIC_DRAW,6,6*sizeof(u16),t_index_data,t_index_dsp);
//    t_mesh->setIndexDsp(t_index_dsp);
//    //
//    IMIBufferDspPtr t_vert_dsp= MakeSharedPtr<IMIBufferDsp>(E_BFM_AOS);
//    IMIRenderMesh::buildIMIBufferDsp(E_BFT_STATIC_DRAW,4,6*sizeof(u16),t_verts,t_vert_dsp);
//    t_mesh->setVertDsp(t_vert_dsp);
//
//    //这个必须有渲染器才可以执行
//    return t_mesh;
...
//    }
//    //
//    IMIRenderMeshPtr t_mesh = _app->getRenderMgr()->createMeshRObj();
...
//菱形
IMIRenderMeshPtr IMIGeoGen::genLing(IMIInstPtr _app,
                                      f32 _len,
                                      IMIBoundBox& _aabb) {
...
//五边
IMIRenderMeshPtr IMIGeoGen::genFiveEdage(IMIInstPtr _app,
                                           f32 _len,
                                           IMIBoundBox& _aabb) {
...
//六边
IMIRenderMeshPtr IMIGeoGen::genSixEdage(IMIInstPtr _app,
                                          f32 _len,
                                          IMIBoundBox& _aabb) {
...
//圆形
IMIRenderMeshPtr IMIGeoGen::genCircle(IMIInstPtr _app,
                                        f32 _len,
                                        IMIBoundBox& _aabb) {
    return _getPolygonDiy(_app,36, 0.0f, _len,_aabb);
}

IMIRenderMeshPtr IMIGeoGen::_getPolygonDiy(IMIInstPtr _app,
                                             s32 _edagenum,
                                             f32 _startangle,
                                             f32 _len,
                                             IMIBoundBox& _aabb) {
...
//    verts[0].t0x = 0.5f;
//    verts[0].t0y = 0.5f;
//    verts[_edagenum + 1].x = cos(imi::degToRad(_startangle)) * 0.5f * _len;
//    verts[_edagenum + 1].y = sin(imi::degToRad(_startangle)) * 0.5f * _len;
//    verts[_edagenum + 1].t0x = cos(imi::degToRad(_startangle)) * 0.5f + 0.5f;
//    verts[_edagenum + 1].t0y = 0.5f - sin(imi::degToRad(_startangle)) * 0.5f;
//    //边点
//    for (s32 i = 0; i < _edagenum; i++) {
//        verts[i + 1].x = cos(imi::degToRad(_startangle)) * 0.5f * _len;
//        verts[i + 1].y = sin(imi::degToRad(_startangle)) * 0.5f * _len;
//        verts[i + 1].t0x = cos(imi::degToRad(_startangle)) * 0.5f + 0.5f;
//        verts[i + 1].t0y = 0.5f - sin(imi::degToRad(_startangle)) * 0.5f;
...
//AABB盒
IMIRenderMeshPtr IMIGeoGen::genAABB(IMIInstPtr _app,IMIBoundBox& _aabb){
    //6*8 = 48个索引点
    u16 m_index[36];
    //4*8 = 32个点
    V3_N_T0 m_verts[24];
...
IMIRenderMeshPtr IMIGeoGen::createRectMesh(IMIInstPtr _app,f32 _w ,f32 _h , s32 _wPointCount , s32 _hPointCont){
...
//        }
//    }
//    IMIRenderMeshPtr  pRenderMesh =  _app->getRenderMgr()->createMeshRObj();
...
IMIRenderMeshPtr IMIGeoGen::createNetGrid(IMIInstPtr _app,s32 _size,s32 _axis) {
//    IMIRenderMeshPtr pRenderMesh =  _app->getRenderMgr()->createMeshRObj();
//    //渲染数据
//    V3_T0 pVer[4];
//    if(_axis ==0 ) {
